...
3. **Ownership Paths & Contract:**
   ...
   - **Reward & RewardRedemption Domain Architecture (SEC-P1-02, R1 & R2):**
     - Canonical ownership paths: `Reward -> organization_id` (direct ownership, implements `OrganizationScopedModel`) and `RewardRedemption -> member.organization_id` (relational ownership, implements `OrganizationScopedModel`).
     - Centralized Global Permissions: `OrganizationScopedModel` only defines the ownership path (`organizationScopePath(): string`). Global domain permissions reside exclusively in `OrganizationScopeService::GLOBAL_PERMISSIONS` (`Reward::class => 'view_cooperative_all'`); model-level magic methods are prohibited.
     - Core Invariant: For every redemption, `member.organization_id` must exist, `reward.organization_id` must exist, and `(string) reward.organization_id === (string) member.organization_id`. `null == null` strictly fails closed.
     - Strict Execution Ordering in `PointService::redeem()`: Cheap tenant equality precondition -> `syncPosPoints($member)` -> pessimistic row lock on `Reward` -> authoritative tenant equality re-check -> stock/balance checks -> mutations and notification dispatch.
     - Admin endpoints: Scoped via `scopeVisibleTo()` (`RewardController::index`, `RewardRedemptionController::index`) and `resolveVisible()` with raw route IDs (`RewardController::update/destroy`, `RewardRedemptionController::show/updateStatus`), returning 404 Not Found on foreign records. Global admin store prohibits orphan reward creation (`organization_id = null`).
     - Member self-service catalog: `GET /api/v1/rewards` and `GET /member/rewards` filter rewards strictly by `where('organization_id', $member->organization_id)`. Null-org members are rejected with 403 Forbidden before catalog queries or point sync.
     - Member self-service redemption: Resolved strictly via `where('organization_id', $member->organization_id)->findOrFail($rewardId)` before calling `PointService::redeem()`.
     - Domain service defense in depth: `PointService::redeem()` enforces tenant checks both before `syncPosPoints()` and after row-level locking of the reward record, preventing side effects on pending POS points, member balances, stock, or outbox notifications.
     - Client forgery prevention: `'organization_id' => ['prohibited']` in `UpdateRedemptionStatusRequest` and `UpdateRewardRequest`; unit-scoped actors cannot set foreign `organization_id` in `StoreRewardRequest`.
