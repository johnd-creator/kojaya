...
- Eliminated cross-domain permission confusion in `OrganizationScopedQueryService` by delegating model-specific global permissions directly to `OrganizationScopeService` without forcing cooperative global permissions on foreign domain models.
- Maintained centralized, explicit global permission registry in `OrganizationScopeService::GLOBAL_PERMISSIONS` (30 model mappings) and added read-only accessor `registeredGlobalPermissions()`.
- Implemented `OrganizationScopedModel` contract on representative models (`Employee`, `CooperativeMember`, `RewardRedemption`).
- Added comprehensive test suite `OrganizationIsolationFoundationTest` with 20 tests (243 assertions) verifying all isolation rules, cross-domain isolation, parent-child security, mutation denial with state preservation, forgery prevention, and full registry schema integrity across all 38 registered models.
- Reconciled inventory truth: 38 registered paths, 30 registered global permissions, 3 representative model contracts, and explicit fail-closed status for unsupported POS models.
- Full regression completed with zero failures: 1,748 passed, 5 skipped, 11,599 assertions.
...
| Jul 15, 2026 | Document 05 Independent Review Remediation | Engineering | Replaced false-confidence source-string assertions with runtime route/service/export/token contracts; closed Project Finance and notification limit bypasses; made member-access audit and token deletion transactional; cleaned up failed sensitive exports; published exact member/loan/invoice/payment OpenAPI schemas and contract evidence. |
| Jul 16, 2026 | Document 05 Remediation Continuation | Engineering | Closed fail-closed audit redaction and actor-context gaps; made privileged deletion and member account link/unlink completion audits transactional; replaced false-confidence rollback tests with real mutation paths; expanded pagination architecture/runtime boundaries and sensitive-data clearing coverage. Local verification: 166 focused passes and 1,237 full-suite passes with 5 pre-existing skips. |
| Jul 20, 2026 | Member Store Account Business Clarification | Engineering | Removed checkout PIN/delegate credential flow, required purchaser attribution, persisted immutable purchaser/cashier/time snapshots through purchase/refund/void, added owner-scoped web member Saldo Toko UX, and published stable mobile API/OpenAPI fields. Backend focused verification: 76 StoreCredit tests plus portal/API regression tests; Android synchronization follows the final backend contract. |
