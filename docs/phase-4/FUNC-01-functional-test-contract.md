...
| **PAY-006** | Payments | Manual Payment Proof Upload by Member | Anggota | `POST /member/payments/proof`, `POST /api/v1/member/payments/proof`, `GET /cooperative/payments/{payment}/proof`, `GET /api/v1/member/payments/{payment}/proof` | `auth:web`/`auth:sanctum`, `member`, `manage_cooperative_payment` | `CooperativePayment`, Private Storage | Uploads image/pdf receipt proof to private disk with status `PENDING`; provides authorized streaming download endpoints for staff and owning member | `Phase1MemberSelfServiceApiTest`, `ContributionsDuesPaymentsFunctionalTest`, `OrganizationPermissionIsolationFunctionalTest::test_pay006_payment_proof_storage_and_authorized_download_access` | None (Fully covered & hardened in FUNC-10: proof upload moved to private disk [local], authenticated download routes with strict policy ownership and cross-org/cross-member denial [403], backward compatibility for legacy public disk proofs, rejection of disallowed executable mime types [.php] and oversized files [>4MB] with zero stored artifacts) | P2 | FUNC-05 | **COVERED** |
...
### RESOLVED FINDING: PAY-006 Public Disk Storage for Payment Proofs (Resolved in FUNC-10)
- **Status**: Resolved in FUNC-10.
- **Resolution**: Configured `filesystems.payment_proof_disk` defaulting to `'local'`. Migrated proof uploads across `MemberSelfServiceController`, `MemberPortalController`, `CooperativePaymentController`, and `CooperativePaymentApiController` to private storage (`payment_proof_disk`, default `local`). Added authorized download routes (`/cooperative/payments/{payment}/proof`, `/api/v1/member/payments/{payment}/proof`, `/member/payments/{payment}/proof`) guarded by `viewProof` policy in `CooperativePaymentPolicy` with backward-compatibility fallback to `public` disk for legacy files. Verified by `OrganizationPermissionIsolationFunctionalTest::test_pay006_payment_proof_storage_and_authorized_download_access`, `ContributionsDuesPaymentsFunctionalTest`, and `Phase1MemberSelfServiceApiTest`.
