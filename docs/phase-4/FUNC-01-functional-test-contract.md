...
| **PAY-006** | Payments | Manual Payment Proof Upload by Member | Anggota | `POST /member/payments/proof`, `POST /api/v1/member/payments/proof`, `GET /cooperative/payments/{payment}/proof`, `GET /member/payments/{payment}/proof`, `GET /api/v1/member/payments/{payment}/proof` | `auth:web`/`auth:sanctum`, `member`, `manage_cooperative_payment` | `CooperativePayment`, Private Storage | Uploads image/pdf receipt proof to private disk with status `PENDING`; provides authorized streaming download endpoints for staff and owning member | `Phase1MemberSelfServiceApiTest`, `ContributionsDuesPaymentsFunctionalTest`, `OrganizationPermissionIsolationFunctionalTest::test_pay006_payment_proof_storage_and_authorized_download_access`, `PaymentProofPrivateStorageMigrationTest` | Historical public files can be migrated by `payments:migrate-proofs-to-private`; deployment migration and public-fallback disablement remain outstanding. Historical files may remain reachable under `/storage/...` until the command is executed and verified. | P2 | FUNC-10 | **PARTIAL** |
...
| **LOAN-010** | Loans | Loan Bad Debt & Write-Off Audit | Pengurus Koperasi | `POST /cooperative/loans/{loan}/write-off` (`LoanController@writeOff`) | Web policy `approve_cooperative_loan` with tenant isolation (`visibleTo`) | `Loan`, `ApprovalLog`, `AuditLog`, `CooperativeLedgerEntry` | Sets `ACTIVE`/`DEFAULTED` → `WRITTEN_OFF`, records reason/note in approval log and audit trail, posts one `LOAN_WRITE_OFF` credit ledger, and rejects replay/unauthorized | `LoanWriteOffAuditLifecycleTest`, `LoanFunctionalTest::test_loan010_authorized_pengurus_can_write_off_active_and_defaulted_loan_with_ledger_and_audit` | **Gap closed**: HTTP route wires Pengurus authorization, reason validation, atomic approval/audit/single ledger credit effect, and 403 denial for Admin, Kasir, and cross-organization actors | P0 | FUNC-08 | **COVERED** |
| **FIN-001** | Finance | Cooperative General Ledger Entry Listing | Admin / Pengurus | `GET /cooperative/ledger` (`CooperativeLedgerController@index`) | `view_cooperative_ledger` | `CooperativeLedgerEntry` | Renders paginated ledger, filters by scope, member, category, entry_type, date range, and computes scoped category summary | `FinanceLedgerFunctionalTest::test_fin001_general_ledger_listing_filters_and_scoped_summary`, `FinanceLedgerFunctionalTest::test_fin001_pos_ledger_scope_filtering_isolation_and_authorization_capabilities`, `FinanceLedgerFunctionalTest::test_fin001_pos_sale_vs_daily_closing_presentation_and_sales_report_isolation` | None (Fully covered: verified paginated ledger filtering by scope [SAVINGS, POS, LOAN], member, category, entry_type, date range, scoped category summary calculation, multi-tenant organization boundaries, scope-aware UI labels [no false 'Total Simpanan' in POS/LOAN], POS sale vs daily closing control entry isolation, PosSalesReportService non-duplication invariant [Rp100k sale not presented as Rp200k], and canCorrectLedgerPayment capability matrix [true strictly for System Admin]) | P2 | FUNC-09 | **COVERED** |
| **FIN-002** | Finance | Ledger Payment Cancellation Workflow | System Admin | `POST /cooperative/ledger/{entry}/cancel-payment` (`CooperativeLedgerController@cancelPayment`) | `manage_cooperative_ledger` with System Admin authorization check | `CooperativeLedgerEntry`, `CooperativePayment`, `CooperativeDuesInvoice`, `ApprovalLog`, `AuditLog` | Destructively deletes ledger entry per ADR-041, marks payment `VOID`, clears receipt, restores invoice `paid_amount`, and records approval log and structured audit trail | `CooperativeFeatureTest`, `FinanceLedgerFunctionalTest::test_fin002_system_admin_can_cancel_payment_with_invoice_restoration_and_audit`, `FinanceLedgerFunctionalTest::test_fin002_payment_cancellation_guards_and_zero_mutation` | None (Covered by automated evidence & ADR-041: formal architectural decision establishes authorized destructive subledger entry deletion with parent payment VOID, receipt deletion, invoice restoration to UNPAID/PARTIAL, locked period and bank reconciliation guards, 403 denial for non-System Admin staff roles, and 100% reconstructable audit log [payment ID, member ID, invoice ID, actor ID, occurred_at timestamp, old/new status, amounts, dates, methods, notes, and reason]) | P0 | FUNC-09 | **COVERED** |
| **FIN-003** | Finance | Ledger Payment Revision Workflow | System Admin | `POST /cooperative/ledger/{entry}/revise-payment` (`CooperativeLedgerController@revisePayment`) | `manage_cooperative_ledger` with System Admin authorization check | `CooperativeLedgerEntry`, `CooperativePayment`, `CooperativeDuesInvoice`, `ApprovalLog`, `AuditLog` | Performs in-place revision of payment and ledger entry credit per ADR-041, adjusts invoice paid_amount by delta, reissues receipt, and writes approval and structured audit trail | `CooperativeFeatureTest`, `FinanceLedgerFunctionalTest::test_fin003_system_admin_can_revise_payment_with_audit_trail`, `FinanceLedgerFunctionalTest::test_fin003_payment_revision_guards_and_zero_mutation` | None (Covered by automated evidence & ADR-041: formal architectural decision establishes authorized in-place payment and ledger entry modification, delta invoice adjustment, receipt reissuance, fixed POKOK/WAJIB amount validation, locked period guards across both old and new dates, 403 denial for non-System Admin staff roles, and 100% reconstructable audit log [payment ID, member ID, invoice ID, actor ID, occurred_at timestamp, old/new status, amounts, dates, methods, notes, and reason]) | P0 | FUNC-09 | **COVERED** |
| **FIN-004** | Finance | Opening Balance Wizard Upload, Preview & Draft | Admin / Pengurus | `GET /cooperative/members/{member}/opening-balance`, `POST .../preview`, `POST .../draft` (`CooperativeOpeningBalanceWizardController@show`, `preview`, `store`) | `manage_cooperative_opening_balance` | `CooperativeMemberOpeningBalanceBatch`, `CooperativeMemberOpeningBalanceLine` | Renders wizard UI, calculates preview amounts without DB persistence, and stores batch with status `DRAFT` and lines with ZERO live ledger mutations | `OpeningBalanceWizardTest`, `OpeningBalanceWizardHttpTest`, `FinanceLedgerFunctionalTest::test_fin004_opening_balance_wizard_preview_and_draft_without_ledger_mutation` | None (Direct HTTP test verifies preview calculations, conflict detection, draft batch persistence, zero ledger mutations at draft stage, non-eligible member validation, and cross-organization boundary 404 denial) | P1 | FUNC-09 | **COVERED** |
| **FIN-005** | Finance | Opening Balance Batch Post to Ledger | Pengurus Koperasi | `POST /cooperative/opening-balances/{batch}/post` (`CooperativeOpeningBalanceWizardController@post`) | `approve_cooperative_opening_balance` | `CooperativeMemberOpeningBalanceBatch`, `CooperativeMemberOpeningBalanceLine`, `CooperativeLedgerEntry` | Commits draft opening balance lines into live `OPENING_BALANCE` credit entries in ledger; enforces replay guard on already processed batches | `OpeningBalanceWizardTest`, `OpeningBalanceWizardHttpTest`, `FinanceLedgerFunctionalTest::test_fin005_opening_balance_post_creates_ledger_entries_with_idempotency` | None (Direct HTTP test verifies line-by-line OPENING_BALANCE credit ledger creation, polymorphic line source uniqueness, replay rejection 422 on already posted batch with zero duplicate entries, and 403 denial for unauthorized actors) | P0 | FUNC-09 | **COVERED** |
| **FIN-006** | Finance | Opening Balance Voiding & Compensating Reversal | Pengurus Koperasi | `POST /cooperative/opening-balances/{batch}/void` (`CooperativeOpeningBalanceWizardController@void`) | `void_cooperative_opening_balance` | `CooperativeMemberOpeningBalanceBatch`, `CooperativeLedgerEntry` | Voids posted batch and creates compensating `OPENING_BALANCE_REVERSAL` debit entries restoring member net opening balance to zero | `OpeningBalanceWizardTest`, `OpeningBalanceWizardHttpTest`, `FinanceLedgerFunctionalTest::test_fin006_opening_balance_void_creates_compensating_reversals_and_restores_balance` | None (Direct HTTP test verifies VOIDED batch transition, reason logging, compensating reversal debit creation matching line amount, mathematical zero balance restoration, rejection of draft/already-voided batches, and 403 denial for unauthorized actors) | P0 | FUNC-09 | **COVERED** |
| **FIN-007** | Finance | Annual SHU Period Calculation, Close & Revision | Pengurus Koperasi | `GET /cooperative/shu`, `POST /cooperative/shu/close`, `POST /cooperative/shu/{period}/request-revision` (`AnnualShuController`) | `manage_cooperative_shu` | `CooperativeShuPeriod`, `CooperativeShuAllocation`, `PosMemberPoint` | Computes SHU allocations based on membership months + paid dues (`shu_score`) and POS profit points (`pos_points`); stores authoritative allocation records; supports revision workflow | `CooperativeFeatureTest`, `FinanceLedgerFunctionalTest::test_fin007_annual_shu_preview_close_and_revision_with_reconciled_allocations` | None (Reconciled contract drift: SHU close creates authoritative allocation records without automatic posting to member ledger or bank accounts; verified formula preview, close allocation creation, repeated close rejection, revision reopening, and re-closing to CLOSED_REVISED) | P1 | FUNC-09 | **COVERED** |
| **FIN-008** | Finance | Member Savings Withdrawal Lifecycle & Race Protection | Member / Pengurus | `POST /api/v1/member/savings/withdraw`, `POST /cooperative/savings/withdrawals/{withdrawal}/process` | Sanctum `member:write` (request) / `manage_cooperative_ledger` (approval) | `SavingsWithdrawal`, `CooperativeLedgerEntry` | Member requests withdrawal up to voluntary balance; staff approval locks row and rechecks voluntary balance (prevents race/overdraft) and creates exact one `SAVING_WITHDRAWAL` debit entry | `SavingsWithdrawalApprovalTest`, `FinanceLedgerFunctionalTest::test_fin008_savings_withdrawal_full_lifecycle_and_balance_race_protection`, `FinanceLedgerFunctionalTest::test_fin008_withdrawal_queue_web_page_rendering_and_pagination`, `FinanceLedgerFunctionalTest::test_fin008_withdrawal_approval_and_rejection_http_dialog_validation` | None (Direct HTTP test verifies member request validation, PENDING status with zero ledger mutation, staff approval with single debit entry, race condition balance recheck preventing overdraft, rejection flow, operational table UI with responsive cards, approve confirmation modal with voluntary balance projection, reject modal with mandatory reason validation, pagination controls, and cross-organization 403 denial) | P0 | FUNC-09 | **COVERED** |
| **ISO-001** | Isolation | Multi-Tenant Organization Boundary on Members & Finance | Staff / Tenant | `GET /cooperative/members`, `GET /cooperative/payments`, `GET /cooperative/loans`, `GET /cooperative/ledger` | `auth`, `OrganizationScopeService` | `CooperativeMember`, `CooperativePayment`, `Loan`, `CooperativeLedgerEntry` | Scopes queries strictly to authenticated user's organization; global permission allows cross-org view; unattached user fails closed | `OrganizationIsolationTest`, `OrganizationPermissionIsolationFunctionalTest::test_iso001_multi_tenant_data_isolation_for_all_cooperative_modules` | Cross-tenant record leakage | P0 | FUNC-10 | **COVERED** |
| **ISO-002** | Isolation | Subsidiary Branch (KBU-001) Zero Member Invariant | All Actors | `GET /cooperative/members`, `POST /cooperative/members` | Role / Tenant Guard | `CooperativeMember`, `Organization`, `CooperativeLedgerEntry` | Enforces structural invariant: KBU-001 commercial subsidiary has strictly 0 members and 0 cooperative financial ledger records | `SeedIntegrityGateTest`, `OrganizationIsolationTest`, `OrganizationPermissionIsolationFunctionalTest::test_iso002_subsidiary_kbu001_zero_member_and_zero_finance_invariant` | Attempted direct DB creation or API insert under KBU-001 | P0 | FUNC-10 | **COVERED** |
| **ISO-003** | Isolation | Cross-Tenant Mutation Rejection & Zero Mutation Standard | External Tenant | Mutating Cooperative Endpoints | Tenant Scoping, Policies | All Cooperative Models, `CooperativeLedgerEntry` | Rejects cross-tenant mutation attempts (payment approval, member edit, loan write-off) with 403/404, strictly guaranteeing zero DB mutations | `SeedIntegrityGateTest`, `OrganizationIsolationTest`, `OrganizationPermissionIsolationFunctionalTest::test_iso003_cross_tenant_mutation_rejection_and_zero_mutation_standard` | Cross-tenant tampering and silent mutation | P0 | FUNC-10 | **COVERED** |
| **ISO-004** | Isolation | POS Product & Category Tenant Scoping | Kasir / Admin | `GET /cooperative/pos-products`, `POST /cooperative/pos-products` | `access_cooperative_pos`, `PosProductPolicy` | `PosProduct`, `PosCategory` | Scopes inventory and catalog strictly to organization; denies cross-org show/update/checkout | `PosProductOrganizationIsolationTest`, `PosCategoryOrganizationIsolationTest`, `OrganizationPermissionIsolationFunctionalTest::test_iso004_pos_product_and_category_tenant_scoping_and_barcode_collision` | Cross-tenant product tampering or checkout collision | P1 | FUNC-10 | **COVERED** |
| **ISO-005** | Isolation | Store Credit Cross-Tenant Boundary | Staff / Member | `GET /cooperative/store-credit/{account}`, `POST .../cash-funding`, `POST .../delegates` | `view_store_credit`, `cashier_store_credit`, `manage_store_credit` | `MemberStoreAccount`, `MemberStoreDelegate` | Blocks cross-organization viewing, cashier cash funding, or delegate assignment with 403/404; guarantees zero balance mutation | `StoreCreditOrganizationScopeTest`, `StoreCreditDelegateOrganizationIsolationTest`, `OrganizationPermissionIsolationFunctionalTest::test_iso005_store_credit_cross_tenant_boundary` | Cross-organization delegate or cash funding tampering | P0 | FUNC-10 | **COVERED** |
| **ISO-006** | Isolation | Member Portal Self-Service Tenant Boundary | Anggota | `GET /member/*`, `GET /api/v1/member/*`, `POST /api/v1/member/payments/proof` | `EnsureIsMember`, `EnsureApiMember` | `CooperativeMember`, `CooperativeDuesInvoice`, `Loan` | Restricts member strictly to own records; rejects accessing another member's invoice, loan, or uploading payment proof for another member | `Phase1MemberSelfServiceApiTest`, `OrganizationPermissionIsolationFunctionalTest::test_iso006_member_portal_self_service_tenant_boundary` | Member A altering request ID to fetch or mutate Member B's records | P0 | FUNC-10 | **COVERED** |
| **ISO-007** | Isolation | Staff Role Privilege Escalation Defense | Admin Koperasi | Staff Endpoints (`loans.approve`, `loans.write-off`, `store-credit.limit`, `ledger.cancel-payment`) | Spatie Permissions, Policies | `Role`, `Permission`, `Loan`, `MemberStoreAccount`, `CooperativeLedgerEntry` | Prevents lower staff role (Admin Koperasi) from executing Pengurus-only actions (loan approval, write-off, credit limit change, ledger cancel) with 403 Forbidden | `RolePermissionMatrixTest`, `CooperativeLoanFeatureTest`, `OrganizationPermissionIsolationFunctionalTest::test_iso007_staff_role_privilege_escalation_defense` | Admin Koperasi attempting privilege escalation | P0 | FUNC-10 | **COVERED** |
| **ISO-008** | Isolation | POS Void, Daily Closing & Shift Diff Fail-Closed Isolation | Kasir / Manajer | `POST /cooperative/pos/void-requests`, `GET /cooperative/pos/closings` | `approve_pos_void`, `OrganizationScopeService` | `PosTransaction`, `PosVoidRequest`, `PosDailyClosing`, `CooperativeLedgerEntry` | Enforces organization boundary on void requests and daily closings; POS_SHIFT_DIFF fails closed (returns null, 0 ledger rows) when organization is unresolved | `PosTransactionVoidOrganizationIsolationTest`, `PosDailyClosingOrganizationIsolationTest`, `OrganizationPermissionIsolationFunctionalTest::test_iso008_pos_void_and_daily_closing_isolation`, `OrganizationPermissionIsolationFunctionalTest::test_pos_shift_diff_fail_closed_edge_cases_a_b_c_d` | Cross-tenant void or closing execution, orphaned shift ledger leak | P0 | FUNC-10 | **COVERED** |
...
### PARTIAL FINDING: PAY-006 Public Disk Storage for Payment Proofs
- **Status**: Partial. New proof uploads use `filesystems.payment_proof_disk` (default `local`) and protected staff, member-web, and member-API download routes enforce authorization. Legacy uploads used `cooperative/payment-proofs/...`; the normal `public/storage` symlink maps that directory to `storage/app/public`, where static web serving bypasses those routes.
- **Migration**: `php artisan payments:migrate-proofs-to-private` is a read-only dry run. After deployment and an approved backup, run `php artisan payments:migrate-proofs-to-private --execute`; it copies only `CooperativePayment.proof_path` values in the payment-proof namespace, verifies SHA-256 integrity and private presence before deleting a public source, inventories unreferenced files without changing them, and reports `migrated`, `already_private`, `missing`, `failed`, `public_removed`, and `public_orphans`. Missing files, conflicts, failed operations, or public orphans return non-zero; uncertain and unreferenced public files remain in place for review.
- **Deployment order**: deploy the private-storage-capable application with the temporary fallback enabled; inspect the dry-run totals; run the explicit migration and require `missing=0`, `failed=0`, and `public_orphans=0`; confirm a second dry run reports `would_remove_public=0` and `public_orphans=0`; set `PAYMENT_PROOF_LEGACY_PUBLIC_FALLBACK=false` and reload cached configuration. Until those deployment steps are completed, historical public files may remain exposed and the fallback remains transitional.
- **Verification**: `PaymentProofPrivateStorageMigrationTest` covers migration, integrity conflicts, missing files, idempotence, dry-run safety, and orphan preservation. `OrganizationPermissionIsolationFunctionalTest::test_pay006_payment_proof_storage_and_authorized_download_access` verifies that migrated files remain downloadable through authorized member and staff endpoints and are absent from the public disk.

### DISPOSED FINDING: STORE-008 Automatic Balance Self-Healing / Reconstruction (Closed as By-Design in FUNC-09)
