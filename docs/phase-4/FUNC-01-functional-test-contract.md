...
| **STORE-008** | Store Credit | Ledger Balance Reconstruction Audit | System | Service (`StoreCreditLedgerService`) | Internal Service | `MemberStoreLedgerEntry`, `MemberStoreAccount` | Recomputes cached balance from full ledger history and enforces strict invariant | `StoreCreditLedgerTest`, `MemberStoreLedgerSchemaRepairTest`, `StoreCreditFunctionalTest` | None (Fully covered: mathematical balance invariant balance == sum(credits) - sum(debits) verified across full lifecycle, ledger entry immutability preventing update and deletion, reversal entry workflow with idempotency restoring mathematical balance, strict drift detection throwing RuntimeException and rolling back transaction on balance mismatch) | P0 | FUNC-06 | **COVERED** |
