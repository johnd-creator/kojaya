...
| **FIN-001** | Finance | Cooperative General Ledger Entry Listing | Admin / Pengurus | `GET /cooperative/ledger` (`CooperativeLedgerController@index`) | `view_cooperative_ledger` | `CooperativeLedgerEntry` | Renders paginated ledger, filters by scope, member, category, entry_type, date range, and computes scoped category summary | `FinanceLedgerFunctionalTest::test_fin001_general_ledger_listing_filters_and_scoped_summary` | None (Direct HTTP test verifies paginated ledger filtering by member, category, entry_type, date range, scoped category summary calculation, organization boundary exclusion, and 403 denial for unauthorized users) | P2 | FUNC-09 | **COVERED** |
| **FIN-002** | Finance | Ledger Payment Cancellation Workflow | System Admin | `POST /cooperative/ledger/{entry}/cancel-payment` (`CooperativeLedgerController@cancelPayment`) | `manage_cooperative_ledger` with System Admin authorization check | `CooperativeLedgerEntry`, `CooperativePayment`, `CooperativeDuesInvoice`, `ApprovalLog`, `AuditLog` | Destructively deletes ledger entry per established product design, marks payment `VOID`, clears receipt, restores invoice `paid_amount`, and records approval log and structured audit trail | `CooperativeFeatureTest`, `FinanceLedgerFunctionalTest::test_fin002_system_admin_can_cancel_payment_with_invoice_restoration_and_audit`, `FinanceLedgerFunctionalTest::test_fin002_payment_cancellation_guards_and_zero_mutation` | None (Reconciled contract drift: product design uses destructive entry deletion + VOID rather than reversal entry; verified invoice restoration, receipt deletion, approval/audit logs, locked period guards, and 403 denial for non-System Admin / Admin Koperasi / Pengurus) | P0 | FUNC-09 | **COVERED** |
| **FIN-003** | Finance | Ledger Payment Revision Workflow | System Admin | `POST /cooperative/ledger/{entry}/revise-payment` (`CooperativeLedgerController@revisePayment`) | `manage_cooperative_ledger` with System Admin authorization check | `CooperativeLedgerEntry`, `CooperativePayment`, `CooperativeDuesInvoice`, `ApprovalLog`, `AuditLog` | Performs in-place revision of payment and ledger entry credit, adjusts invoice paid_amount by delta, reissues receipt, and writes approval and structured audit trail | `CooperativeFeatureTest`, `FinanceLedgerFunctionalTest::test_fin003_system_admin_can_revise_payment_with_audit_trail`, `FinanceLedgerFunctionalTest::test_fin003_payment_revision_guards_and_zero_mutation` | None (Reconciled contract drift: product design modifies existing entry credit in-place rather than posting delta entry; verified delta invoice adjustment, receipt reissue, audit logging with old/new values, fixed POKOK/WAJIB amount validation, locked period guards, and 403 denial for non-System Admin) | P0 | FUNC-09 | **COVERED** |
