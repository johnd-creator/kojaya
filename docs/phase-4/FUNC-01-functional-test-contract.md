...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :---: | :---: | :---: |
| **AUTH-001** | Auth | Web Login & Role-Based Redirection | All Users | `POST /login` (`AuthenticatedSessionController@store`) | Public / Web Guest | `User`, `CooperativeMember`, `Role` | Session created, audit log, redirection based on role/member status | `AuthenticationTest`, `P5MemberAuthRedirectTest` | Redirect edge cases when user has inactive member status | P1 | FUNC-02 | **COVERED** |
| **AUTH-002** | Auth | Web Logout & Session Invalidation | Auth Users | `POST /logout` (`AuthenticatedSessionController@destroy`) | `auth:web` | `Session` | Session destroyed, CSRF token regenerated | `AuthenticationTest` | Concurrent session invalidation | P2 | FUNC-02 | **COVERED** |
| **AUTH-003** | Auth | Two-Factor Authentication (2FA) Setup & Challenge | Staff / Members | `POST /user/two-factor-authentication`, `POST /two-factor-challenge` | `auth:web`, `password.confirm` | `User` (2FA secrets/recovery codes) | 2FA enabled, secret generated, recovery codes stored | `TwoFactorAuthenticationTest` | Member portal 2FA challenge flow | P1 | FUNC-02 | **PARTIAL** |
| **AUTH-004** | Auth | Google SSO Redirection & Callback | Anggota | `GET /auth/google/redirect`, `GET /auth/google/callback` | Public / Web Guest | `User`, `SocialAccount`, `CooperativeMember` | Creates or links social account, initiates member session | `GoogleSsoFlowTest`, `GoogleSsoMemberMatchingTest`, `GoogleSsoOpenRedirectHardeningTest` | Error handling on invalid/tampered OAuth state parameter | P1 | FUNC-02 | **COVERED** |
| **AUTH-005** | Auth | Mobile API Token Issuance (Password) | Mobile Users | `POST /api/auth/login` (`AuthController@login`) | Public / API Guest | `User`, `PersonalAccessToken` | Issues Sanctum token with `TokenApp` abilities | `MemberTokenAbilityTest`, `AuthControllerTest` | Rate limiting on brute force API login attempts | P1 | FUNC-02 | **PARTIAL** |
| **AUTH-006** | Auth | Inactive / Blocked Member Access Denial | Anggota | `POST /login`, `POST /api/auth/login` | Fortify / Sanctum | `User`, `CooperativeMember` | 403 Forbidden / Login rejection with exact error code | `MemberFirstLoginLifecycleExperienceTest`, `MemberTokenAbilityTest` | Token revocation when member status becomes RESIGNED/EXPELLED mid-session | P0 | FUNC-02 | **PARTIAL** |
| **AUTH-007** | Auth | API Token Revocation (Logout) | Mobile Users | `POST /api/auth/logout` (`AuthController@logout`) | `auth:sanctum` | `PersonalAccessToken` | Token revoked/deleted from DB | `MemberLifecycleTokenRevocationTest` | Bulk token revocation across all devices | P2 | FUNC-02 | **COVERED** |
| **AUTH-008** | Auth | Password Confirmation & Reset Link | All Users | `POST /forgot-password`, `POST /reset-password` | Public / Guest | `User`, `password_reset_tokens` | Reset token generated, email dispatched | `PasswordResetTest`, `PasswordConfirmationTest` | Password reset token expiration boundary | P2 | FUNC-02 | **COVERED** |
| **MEM-001** | Lifecycle | Member Onboarding View & Eligibility | Anggota | `GET /member/onboarding` (`MemberPortalController@onboarding`) | `auth:web`, `member` | `CooperativeMember`, `User` | Renders onboarding steps based on current lifecycle stage | `MemberOnboardingAccessTest` | Onboarding view access for already ACTIVE members (should redirect) | P2 | FUNC-03 | **COVERED** |
| **MEM-002** | Lifecycle | Member Onboarding Form Submission | Anggota | `POST /member/onboarding` (`MemberPortalController@submitOnboarding`) | `auth:web`, `member` | `CooperativeMember`, `User` | Updates NIK, phone, address, sets state to `WAITING_VERIFICATION` | `MemberOnboardingAccessTest`, `P1FormRequestValidationTest` | Duplicate NIK submission handling across organizations | P0 | FUNC-03 | **PARTIAL** |
| **MEM-003** | Lifecycle | Member Profile Retrieval (Self-Service) | Anggota | `GET /member/profile`, `GET /api/v1/member/profile` | `auth:web`/`auth:sanctum`, `member.active` | `CooperativeMember`, `User` | Returns member profile, savings summary, badges | `Phase1MemberSelfServiceApiTest` | Profile completeness percentage calculation accuracy | P3 | FUNC-03 | **COVERED** |
| **MEM-004** | Lifecycle | Member Profile Update (Phone / Address) | Anggota | `PUT /member/profile`, `PUT /api/v1/member/profile` | `auth:web`/`auth:sanctum`, `member.active` | `CooperativeMember` | Updates editable fields, logs audit change | `Phase1MemberSelfServiceApiTest` | Tamper attempt on immutable fields (e.g. `member_no`, `status`) | P0 | FUNC-03 | **PARTIAL** |
| **MEM-005** | Lifecycle | Member Status Experience Resolution | Anggota | Internal Service (`MemberLifecycleExperience::fromMember`) | Framework / Policy | `CooperativeMember` | Resolves canonical experience enum | `CooperativeMemberLifecycleSeederTest`, `SeedIntegrityGateTest` | Transition when validation notes are updated without status change | P2 | FUNC-03 | **COVERED** |
| **MEM-006** | Lifecycle | Member Resignation Request Submission | Anggota | `POST /cooperative/members/{member}/resign` | `auth:web`, `member.active` | `MemberResignationRequest`, `CooperativeMember` | Creates pending resignation request | `MemberResignationAuthorizationTest` | Resignation submission while active loans exist (must block) | P0 | FUNC-03 | **PARTIAL** |
| **MEM-007** | Lifecycle | Member Account Link to SSO | Anggota | `POST /cooperative/members/{member}/link-account` | `auth:web`, `manage_cooperative_member` | `SocialAccount`, `CooperativeMember` | Links Google provider ID to member record | `MemberAccountLinkAuthorizationTest` | Linking account already bound to another active member | P0 | FUNC-03 | **COVERED** |
| **MEM-008** | Lifecycle | Member Status Consistency Check | System | Internal Scan (`MemberStatusConsistencyReport`) | Cron / CLI / Test | `CooperativeMember` | Reports inconsistencies between status and lifecycle dates | `MemberStatusConsistencyTest` | Automated resolution of corrupted lifecycle dates | P2 | FUNC-03 | **COVERED** |
| **ADM-001** | Admin Member | Member Directory Listing & Filters | Admin / Pengurus | `GET /cooperative/members` (`CooperativeMemberController@index`) | `view_cooperative_member` | `CooperativeMember` | Renders paginated list, filters by status, branch, search | Visual Tests (`members.visual.spec.ts`) | **Direct HTTP backend test for search, filter query combinations** | P2 | FUNC-04 | **MISSING** |
| **ADM-002** | Admin Member | Member Registration (Admin Direct) | Admin / Pengurus | `POST /cooperative/members` (`CooperativeMemberController@store`) | `manage_cooperative_member` | `CooperativeMember`, `User` | Creates member, generates `member_no`, creates user account | None (Tested via seeders/visual) | **Direct HTTP test verifying validation rules, duplicate check, transaction boundary** | P1 | FUNC-04 | **MISSING** |
| **ADM-003** | Admin Member | Member Detail & Sensitive Data Masking | Admin / Pengurus | `GET /cooperative/members/{member}` (`CooperativeMemberController@show`) | `view_cooperative_member` | `CooperativeMember` | Shows member details, masks NIK/phone if no PII permission | Visual Tests | **Authorization test asserting PII masking for roles without PII view permission** | P0 | FUNC-04 | **MISSING** |
| **ADM-004** | Admin Member | Member Validation Review (Under Review) | Admin Koperasi | `POST /cooperative/members/{member}/validate` | `validate_cooperative_member` | `CooperativeMember`, `ApprovalLog` | Sets validation status to `UNDER_REVIEW`, records reviewer | `CooperativeMemberValidationTest` | Transition attempt from already `ACTIVE` status | P1 | FUNC-04 | **COVERED** |
| **ADM-005** | Admin Member | Member Revision Request | Admin Koperasi | `POST /cooperative/members/{member}/revision` | `validate_cooperative_member` | `CooperativeMember`, `ApprovalLog` | Sets status `REVISION_REQUIRED`, stores notes for member | `CooperativeMemberValidationTest` | Empty notes validation requirement | P2 | FUNC-04 | **COVERED** |
| **ADM-006** | Admin Member | Member Final Approval (Pengurus Only) | Pengurus Koperasi | `POST /cooperative/members/{member}/approve` | `approve_cooperative_member` | `CooperativeMember`, `ApprovalLog` | Sets status `ACTIVE`, assigns `activated_at`, activates account | `CooperativeMemberValidationTest` | Non-pengurus (Admin Kop) attempt to approve (must 403) | P0 | FUNC-04 | **COVERED** |
| **ADM-007** | Admin Member | Member Rejection (Pengurus Only) | Pengurus Koperasi | `POST /cooperative/members/{member}/reject` | `approve_cooperative_member` | `CooperativeMember`, `ApprovalLog` | Sets status `REJECTED`, records rejection reason | `CooperativeMemberValidationTest` | Rejection reason character limit and requiredness | P2 | FUNC-04 | **COVERED** |
| **ADM-008** | Admin Member | Member Batch Import (Upload, Preview, Execute) | Admin Koperasi | `POST /cooperative/members/import/upload`, `POST .../execute` | `import_cooperative_member_batch` | `CooperativeMember`, `User` | Bulk imports members, generates credentials, audits import | `MemberImportPreviewTest`, `MemberImportValidatorTest`, `MemberImportExecutionTest`, `MemberImportConcurrencyTest` | Partial rollback on syntax error in Excel row | P1 | FUNC-04 | **COVERED** |
...
2. **Gap Kontroller Web Admin Anggota (Domain C / `FUNC-04`)**:
   - `CooperativeMemberController` (halaman index, create, store, edit, update, PII unmasking) banyak diuji melalui pengujian visual Playwright, namun pengujian fungsional backend HTTP murni untuk kombinasi pencarian, penyaringan status, validasi form, dan proteksi PII masking masih minim.
