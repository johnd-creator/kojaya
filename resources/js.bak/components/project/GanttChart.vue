...
const attachEvents = () => {
    ...
    eventIds.push(
        gantt.attachEvent('onAfterLinkAdd', async (tempId, link) => {
            try {
                const res = await axios.post(`/projects/${props.projectId}/gantt-link`, {
                    source: link.source,
                    target: link.target,
                    type: link.type,
                });
    ...
    eventIds.push(
        gantt.attachEvent('onAfterTaskUpdate', async (id, task) => {
            try {
                const start = toDate((task as any).start_date);
                const end = (task as any).end_date ? toDate((task as any).end_date) : calcEndDate(start, toDuration((task as any).duration));
...
<template>
    <div ref="ganttContainer" class="h-[600px] w-full overflow-hidden rounded-lg border"></div>
</template>
