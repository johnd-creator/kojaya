...
type Employee = { id: number; employee_code: string; first_name: string; last_name?: string; organization_id: string };
        ...
        <div class="flex flex-1 flex-col gap-6 p-6 max-w-3xl mx-auto w-full">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-zinc-900 dark:text-white">Create Transfer</h1>
            ...
            <div class="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6">
                <form @submit.prevent="submit" class="space-y-4">
                    <div class="grid gap-2">
                        ...
                        <select v-model="form.employee_id" class="flex h-10 w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950" required>
                    ...
                    <div class="grid gap-2">
                        ...
                        <select v-model="form.to_organization_id" class="flex h-10 w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950" required>
                            <option value="" disabled>Pilih organisasi tujuan</option>
                            <option v-for="org in organizations" :key="org.id" :value="org.id">
                                {{ org.code ? `${org.code} - ${org.name}` : org.name }}
                            </option>
                        </select>
                        <span v-if="form.errors.to_organization_id" class="text-xs text-red-500">{{ form.errors.to_organization_id }}</span>
                    </div>

                    <div class="grid gap-2">
                        <Label>Effective Date</Label>
                        <Input v-model="form.effective_date" type="date" required />
                        <span v-if="form.errors.effective_date" class="text-xs text-red-500">{{ form.errors.effective_date }}</span>
                    </div>
