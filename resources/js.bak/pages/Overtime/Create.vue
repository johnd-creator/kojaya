        ...
        <div class="flex flex-col gap-6 p-6 max-w-2xl mx-auto w-full">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-zinc-500 mt-1">Submit a new request for overtime work</p>
                </div>
            </div>

            <form @submit.prevent="submit" class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 space-y-6">
                ...
                <div class="grid gap-2">
                    ...
                    <span v-if="form.errors.overtime_rule_id" class="text-xs text-red-500">{{ form.errors.overtime_rule_id }}</span>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="grid gap-2">
                        ...
                        <span v-if="form.errors.start_time" class="text-xs text-red-500">{{ form.errors.start_time }}</span>
                    </div>
                    <div class="grid gap-2">
                        <Label>End Time</Label>
                        <Input type="time" v-model="form.end_time" required />
                        <span v-if="form.errors.end_time" class="text-xs text-red-500">{{ form.errors.end_time }}</span>
                    </div>
                </div>
                ...
                <div class="grid gap-2">
                    ...
                    <div class="flex items-center gap-4">
                        <Input type="file" @input="form.evidence = ($event.target as HTMLInputElement).files?.[0] || null" accept=".jpg,.jpeg,.png,.pdf" />
                    </div>
                    <p class="text-xs text-zinc-500">Upload approval email, timesheet, or photo of work. Max 2MB.</p>
                    <span v-if="form.errors.evidence" class="text-xs text-red-500">{{ form.errors.evidence }}</span>
                </div>
