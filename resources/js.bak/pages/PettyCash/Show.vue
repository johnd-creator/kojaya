...
import { ArrowLeft, Wallet, TrendingUp, TrendingDown, FileText, Calendar, Download } from 'lucide-vue-next';
import { ref } from 'vue';
        ...
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-6 max-w-7xl mx-auto w-full">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">{{ account.name }}</h1>
                        <p class="text-zinc-500 text-sm flex items-center gap-2 mt-1">
                            <span class="font-medium text-indigo-600 dark:text-indigo-400">{{ account.organization.name }}</span>
                            <span>•</span>
                            <span :class="account.status === 'ACTIVE' ? 'text-emerald-600' : 'text-zinc-500'">{{ account.status }}</span>
                        </p>
                    </div>
                </div>
                
                <div class="flex items-center gap-3">
                    <Button @click="isTransactionModalOpen = true" class="bg-indigo-600 hover:bg-indigo-700 text-white">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                    ...
                    <p class="text-3xl font-bold text-zinc-900 dark:text-white mt-2">{{ formatCurrency(account.balance) }}</p>
                </div>
                <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                    <p class="text-sm font-medium text-zinc-500">Limit</p>
                    <p class="text-3xl font-bold text-zinc-900 dark:text-white mt-2">{{ formatCurrency(account.limit) }}</p>
                </div>
                <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                    <p class="text-sm font-medium text-zinc-500">Available Limit</p>
                    <p class="text-3xl font-bold text-zinc-900 dark:text-white mt-2">{{ formatCurrency(account.limit - account.balance) }}</p>
                    <p class="text-xs text-zinc-400 mt-1">Note: This logic depends on policy</p>
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800">
                                ...
                                <th class="py-4 px-6 font-medium text-sm text-zinc-500 uppercase tracking-wider">Reference</th>
                                <th class="py-4 px-6 font-medium text-sm text-zinc-500 uppercase tracking-wider text-right">Amount</th>
                                <th class="py-4 px-6 font-medium text-sm text-zinc-500 uppercase tracking-wider">By</th>
                                <th class="py-4 px-6 font-medium text-sm text-zinc-500 uppercase tracking-wider text-right">Proof</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                            ...
                            >
                                ...
                                <td class="py-4 px-6">
                                    <Badge :variant="trx.type === 'DEBIT' ? 'default' : 'destructive'" class="capitalize">
                                ...
                                <td class="py-4 px-6 text-right font-medium" :class="trx.type === 'DEBIT' ? 'text-emerald-600' : 'text-red-600'">
                                ...
                                <td class="py-4 px-6 text-right">
                                    <a v-if="trx.proof_file" :href="'/storage/' + trx.proof_file" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm inline-flex items-center">
