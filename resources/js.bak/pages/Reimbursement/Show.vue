...
import { ArrowLeft, CheckCircle2, XCircle, DollarSign, FileText, Download, Calendar, User as UserIcon } from 'lucide-vue-next';
import { ref } from 'vue';
...
const approve = () => {
    if (confirm('Are you sure you want to approve this reimbursement?')) {
        router.post(`/reimbursements/${props.reimbursement.id}/approve`, {}, {
            preserveScroll: true,
            onSuccess: () => {
                // Success
            },
...
const pay = () => {
    if (confirm('Are you sure you want to mark this reimbursement as PAID?')) {
        router.post(`/reimbursements/${props.reimbursement.id}/pay`, {}, {
            preserveScroll: true,
            onSuccess: () => {
                // Success
            },
        ...
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-6 max-w-7xl mx-auto w-full">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Reimbursement #{{ reimbursement.id.substring(0, 8) }}</h1>
                ...
                <div class="flex items-center gap-2">
                    <template v-if="can.approve">
                        <Button variant="default" @click="approve" class="bg-emerald-600 hover:bg-emerald-700 text-white">
                        ...
                        <Dialog v-model:open="isRejectDialogOpen">
                            ...
                            <DialogContent>
                                ...
                                <DialogFooter>
                                    <Button variant="outline" @click="isRejectDialogOpen = false">Cancel</Button>
                                    <Button variant="destructive" @click="reject" :disabled="rejectForm.processing">Confirm Rejection</Button>
                                </DialogFooter>
                            </DialogContent>
                        </Dialog>
                    </template>
                    ...
                    <template v-if="can.pay">
                        <Button variant="default" class="bg-green-600 hover:bg-green-700 text-white" @click="pay">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                    ...
                    <div class="mt-2 flex items-center gap-2">
                        <Badge variant="outline" class="text-lg px-3 py-1 uppercase tracking-wide">
                            {{ reimbursement.status }}
                        </Badge>
                    </div>
                    <div v-if="reimbursement.rejection_reason" class="mt-3 text-sm text-destructive bg-destructive/10 p-2 rounded">
                        <span class="font-semibold block">Rejection Reason:</span>
                        {{ reimbursement.rejection_reason }}
                    </div>
                    <div v-if="reimbursement.approver" class="mt-3 text-sm text-zinc-600 dark:text-zinc-400">
                        <span class="font-semibold">Approved by:</span> {{ reimbursement.approver.name }}
                    </div>
                </div>

                <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                    <p class="text-sm font-medium text-zinc-500">Total Amount</p>
                    <p class="text-3xl font-bold text-zinc-900 dark:text-white mt-2">{{ formatCurrency(reimbursement.total_amount) }}</p>
                    <p v-if="reimbursement.payment_date" class="text-xs text-emerald-600 mt-1 flex items-center gap-1">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
                <div class="p-6 border-b border-zinc-200 dark:border-zinc-800">
                    <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Expense Items</h2>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800">
                                ...
                                <th class="py-4 px-6 font-medium text-sm text-zinc-500 uppercase tracking-wider">Receipt</th>
                        ...
                        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                            ...
                            >
                                ...
                                <td class="py-4 px-6 text-sm text-zinc-900 dark:text-zinc-100 max-w-xs truncate">
                                    {{ item.description }}
                                </td>
                                <td class="py-4 px-6">
                                    <a
                        ...
                        <tfoot class="bg-zinc-50 dark:bg-zinc-900/50 border-t border-zinc-200 dark:border-zinc-800">
                            <tr>
                                <td colspan="4" class="py-4 px-6 text-right font-bold text-sm text-zinc-900 dark:text-white uppercase tracking-wider">
                                    Total
                                </td>
                                <td class="py-4 px-6 text-right font-bold text-zinc-900 dark:text-white">
