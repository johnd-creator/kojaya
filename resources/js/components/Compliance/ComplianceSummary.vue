...
import { TrendingUp, TrendingDown, AlertTriangle, CheckCircle, RefreshCw } from 'lucide-vue-next';
import { computed, onMounted, ref, watch } from 'vue';
import { fetchCertificateCompliance, fetchMcuCompliance } from '@/api/compliance.ts';
...
const overallCompliance = computed(() => {
    ...
    const total = certificateReport.value.total_employees + mcuReport.value.total_employees;
    const compliant = certificateReport.value.compliant_employees + mcuReport.value.compliant_employees;
...
const updateCertificatesChart = () => {
    ...
    const valid = certificateReport.value.certificates.filter(c => c.status === 'VALID').length;
    const expired = certificateReport.value.certificates.filter(c => c.status === 'EXPIRED' || c.status === 'REVOKED').length;
...
const updateMcuChart = () => {
    ...
    const fit = mcuReport.value.medical_checkups.filter(m => m.result === 'FIT').length;
    const withRestriction = mcuReport.value.medical_checkups.filter(m => m.result === 'FIT_WITH_RESTRICTION').length;
    const unfit = mcuReport.value.medical_checkups.filter(m => m.result === 'UNFIT').length;
    const totalEmployees = certificateReport.value?.total_employees ?? mcuReport.value.total_employees;
...
<template>
    <div class="space-y-6">
        ...
        <div v-if="error" class="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-900 dark:bg-red-900/20">
                    ...
                    <div v-else-if="certificateReport">
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-neutral-600 dark:text-neutral-400">Total Employees:</span>
                                <span class="font-semibold text-neutral-900 dark:text-neutral-100">
                                    {{ certificateReport.total_employees }}
                                </span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-neutral-600 dark:text-neutral-400">Compliant:</span>
                                <span class="font-semibold text-green-600 dark:text-green-400">
                                    {{ certificateReport.compliant_employees }}
                                </span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-neutral-600 dark:text-neutral-400">Non-Compliant:</span>
        ...
        <Card>
            ...
            <CardContent>
                <div v-if="loading" class="flex items-center justify-center py-4">
                    <div class="h-6 w-6 animate-spin rounded-full border-2 border-neutral-300 border-t-neutral-900 dark:border-neutral-700 dark:border-t-neutral-100" />
                </div>
