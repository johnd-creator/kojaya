...
import { AlertTriangle, FileText, Activity, Download, Filter, RefreshCw } from 'lucide-vue-next';
import { computed, onMounted, ref } from 'vue';
import { fetchNonCompliantEmployees, exportComplianceReport } from '@/api/compliance.ts';
...
const filteredEmployees = computed(() => {
    ...
    if (filters.value.type !== 'all') {
        const issueKey = filters.value.type === 'certificate' ? 'certificate_issues' : 'mcu_issues';
...
const hasIssues = (employee: NonCompliantEmployee): boolean => {
    const certIssues = employee.certificate_issues && employee.certificate_issues.length > 0;
    ...
    <div class="space-y-4">
        <Card>
            <CardHeader>
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="flex items-center gap-2">
                            <Select v-model="filters.type" @update:model-value="fetchEmployees">
            ...
            <CardContent>
                ...
                <div v-else-if="filteredEmployees.length === 0" class="py-12 text-center">
                    <AlertTriangle class="mx-auto mb-4 h-16 w-16 text-neutral-300 dark:text-neutral-700" />
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b border-neutral-200 dark:border-neutral-800">
                                        <th class="px-4 py-3 text-left font-medium text-neutral-700 dark:text-neutral-300">
                                            Employee
                                        </th>
                                        <th class="px-4 py-3 text-left font-medium text-neutral-700 dark:text-neutral-300">
                                            Department
                                        ...
                                        <th class="px-4 py-3 text-right font-medium text-neutral-700 dark:text-neutral-300">
                                            Actions
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    >
                                        ...
                                        <td class="px-4 py-3">
                                            <div v-if="employee.certificate_issues && employee.certificate_issues.length > 0" class="space-y-1">
                                                <Badge
                                                    v-for="(issue, index) in employee.certificate_issues.slice(0, 2)"
                                        ...
                                        <td class="px-4 py-3">
                                            <div v-if="employee.mcu_issues && employee.mcu_issues.length > 0" class="space-y-1">
                                                <Badge
                                                    v-for="(issue, index) in employee.mcu_issues.slice(0, 2)"
