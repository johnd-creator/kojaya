...
import { FileText, Award, AlertTriangle, Plus, Upload, Trash2, Pencil, File } from 'lucide-vue-next';
...
const groupedCertificates = computed(() => {
    return {
        ...
        expired: certificates.value.filter((c) => c.status === 'EXPIRED' || c.status === 'REVOKED'),
    };
});
...
const openUpload = (certificateId: string) => {
    router.visit(`/employees/${props.employeeId}/certificates/${certificateId}/upload`);
};
...
const openEdit = (certificateId: string) => {
    router.visit(`/employees/${props.employeeId}/certificates/${certificateId}/edit`);
};
...
<template>
    <div class="space-y-4">
        ...
        <div v-else-if="!hasCertificates" class="py-12 text-center">
            <Award class="mx-auto mb-4 h-16 w-16 text-neutral-300 dark:text-neutral-700" />
            <h3 class="text-lg font-semibold text-neutral-900 dark:text-neutral-100">
                No certificates yet
            </h3>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center gap-2 text-xs text-neutral-500 dark:text-neutral-500">
                            <span>Issued: {{ formatExpiryDate(certificate.issue_date) }}</span>
                            <span>•</span>
                            <span :class="[
                    ...
                    <DropdownMenu v-if="!readonly">
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                                ...
                                @click="window.open(getDocumentUrl(certificate.document_path), '_blank')"
                            >
