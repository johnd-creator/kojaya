...
import { Calendar, Download, Loader2 } from 'lucide-vue-next'
import { ref, computed, watch } from 'vue'
import {   downloadBlob, generatePayslip, generatePayrollSummary, generatePayrollDetail, generateAttendanceReport, generateLeaveReport, generateCertificateCompliance, generateMcuCompliance } from '@/components/Report/helpers'
...
const canGenerate = computed(() => {
  ...
  if (showDateRangeFilter.value && (!filters.value.date_from || !filters.value.date_to)) return false
  ...
  try {
    ...
    switch (props.report.id) {
      case 'payslip':
        ...
        blob = await generatePayslip(filters.value.employee_id, filters.value.period, selectedFormat.value)
        downloadBlob(blob, `payslip_${filters.value.period}_${filters.value.employee_id}.${selectedFormat.value}`)
        break

      case 'payroll-summary':
        blob = await generatePayrollSummary({ ...filters, format: selectedFormat.value })
        downloadBlob(blob, `payroll_summary_${filters.value.period || 'all'}.${selectedFormat.value}`)
        break

      case 'payroll-detail':
        blob = await generatePayrollDetail({ ...filters, format: selectedFormat.value })
        downloadBlob(blob, `payroll_detail_${filters.value.period || 'all'}.${selectedFormat.value}`)
        break

      case 'attendance':
        blob = await generateAttendanceReport({ ...filters, format: selectedFormat.value })
        downloadBlob(blob, `attendance_${filters.value.date_from || 'all'}_to_${filters.value.date_to || 'all'}.${selectedFormat.value}`)
        break

      case 'leave':
        blob = await generateLeaveReport({ ...filters, format: selectedFormat.value })
        downloadBlob(blob, `leave_${filters.value.date_from || 'all'}_to_${filters.value.date_to || 'all'}.${selectedFormat.value}`)
        break

      case 'certificate-compliance':
        blob = await generateCertificateCompliance({ ...filters, format: selectedFormat.value })
        downloadBlob(blob, `certificate_compliance_${filters.value.date_from || 'all'}_to_${filters.value.date_to || 'all'}.${selectedFormat.value}`)
        break

      case 'mcu-compliance':
        blob = await generateMcuCompliance({ ...filters, format: selectedFormat.value })
        downloadBlob(blob, `mcu_compliance_${filters.value.date_from || 'all'}_to_${filters.value.date_to || 'all'}.${selectedFormat.value}`)
        break
  ...
  <AppLayout>
    <div class="space-y-6 p-6 max-w-4xl mx-auto">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h2 class="text-2xl font-bold tracking-tight">{{ props.report.name }}</h2>
          <p class="text-muted-foreground">{{ props.report.description }}</p>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
        <form @submit.prevent="generateReport" class="space-y-6">
          ...
          <div v-if="showDateRangeFilter" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-2">
          ...
          <div class="flex items-center gap-3 pt-4 border-t">
            ...
            >
              ...
              {{ loading ? 'Generating...' : `Generate ${selectedFormat.toUpperCase()}` }}
            </Button>
