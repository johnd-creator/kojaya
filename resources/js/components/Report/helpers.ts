...
import { payslip, payrollSummary, payrollDetail, attendanceReport, leaveReport, certificateCompliance, mcuCompliance } from '@/actions/App/Http/Controllers/ReportController'
...
export interface Report {
  ...
  category: string
  formats: ('pdf' | 'excel')[]
  filters: ('period' | 'date_from' | 'employee_id' | 'organization_id' | 'unit_id')[]
}
...
export const generatePayslip = async (employeeId: number, period: string, format: 'pdf' | 'excel' = 'pdf'): Promise<Blob> => {
  const projectId = 1
...
export const generatePayrollSummary = async (filters: ReportFilter & { format?: 'pdf' | 'excel' }): Promise<Blob> => {
  const projectId = 1
  const response = await axios({
    method: 'get',
...
export const generatePayrollDetail = async (filters: ReportFilter & { format?: 'pdf' | 'excel' }): Promise<Blob> => {
  const projectId = 1
  const response = await axios({
    method: 'get',
...
export const generateAttendanceReport = async (filters: ReportFilter & { format?: 'pdf' | 'excel' }): Promise<Blob> => {
  const projectId = 1
  const response = await axios({
    method: 'get',
...
export const generateLeaveReport = async (filters: ReportFilter & { format?: 'pdf' | 'excel' }): Promise<Blob> => {
  const projectId = 1
  const response = await axios({
    method: 'get',
...
export const generateCertificateCompliance = async (filters: ReportFilter & { format?: 'pdf' | 'excel' }): Promise<Blob> => {
  const projectId = 1
  const response = await axios({
    method: 'get',
...
export const generateMcuCompliance = async (filters: ReportFilter & { format?: 'pdf' | 'excel' }): Promise<Blob> => {
  const projectId = 1
  const response = await axios({
    method: 'get',
