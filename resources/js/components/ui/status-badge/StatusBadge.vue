...
import { CheckCircle2, XCircle, AlertCircle, Clock, FileText, Ban } from 'lucide-vue-next';
...
const variant = computed(() => {
  ...
  if (['APPROVED', 'PAID', 'COMPLETED', 'ACTIVE', 'PUBLISHED', 'VERIFIED'].includes(status)) {
    return 'success';
  }
  
  if (['REJECTED', 'CANCELLED', 'INACTIVE', 'DENIED', 'FAILED', 'ERROR'].includes(status)) {
    return 'destructive';
  }
  
  if (['PENDING', 'SUBMITTED', 'DRAFT', 'IN_PROGRESS', 'REVIEWING', 'PROCESSING', 'WAITING'].includes(status)) {
...
const icon = computed(() => {
  ...
  if (['APPROVED', 'PAID', 'COMPLETED', 'VERIFIED'].includes(status)) return CheckCircle2;
  if (['REJECTED', 'FAILED', 'ERROR', 'DENIED'].includes(status)) return XCircle;
  if (['CANCELLED', 'INACTIVE', 'BAN'].includes(status)) return Ban;
  if (['PENDING', 'WAITING', 'PROCESSING'].includes(status)) return Clock;
  if (['SUBMITTED', 'IN_PROGRESS', 'REVIEWING'].includes(status)) return AlertCircle;
