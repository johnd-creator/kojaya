...
import { ArrowLeft, Wrench, Calendar, MapPin, Tag, Edit2, Trash2 } from 'lucide-vue-next';
...
const deleteAsset = () => {
    if (confirm(`Are you sure you want to delete ${props.asset.name}?`)) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = `/assets/${props.asset.id}`;
        ...
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-6 max-w-7xl mx-auto w-full">
            ...
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">{{ asset.name }}</h1>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" @click="deleteAsset" class="h-8 w-8 text-red-600 hover:text-red-700 hover:bg-red-50">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
                    <h2 class="text-lg font-semibold text-zinc-900 dark:text-white mb-4">Asset Information</h2>
                    <div class="space-y-4">
                        <div class="flex items-start gap-3">
                            <Tag class="h-5 w-5 text-zinc-400 mt-0.5" />
                            <div class="flex-1">
                                <p class="text-sm text-zinc-500">Category</p>
                                <p class="font-medium text-zinc-900 dark:text-zinc-100">{{ asset.category }}</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1">
                                ...
                                <p v-if="asset.organization" class="text-xs text-zinc-500 mt-0.5">
                        ...
                        <div v-if="asset.serial_number" class="flex items-start gap-3">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="font-mono text-sm text-zinc-900 dark:text-zinc-100">{{ asset.serial_number }}</p>
                            </div>
                        </div>
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Work Orders</h2>
                        <span class="text-sm text-zinc-500">{{ asset.workOrders?.length || 0 }} total</span>
                    </div>
                    <div v-if="!asset.workOrders || asset.workOrders.length === 0" class="text-center py-8 text-zinc-500">
                        ...
                        >
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-mono text-zinc-700 dark:text-zinc-300 mb-1">
                                        {{ wo.id.slice(0, 8) }}
                                    </p>
                                    <p v-if="wo.description" class="text-sm text-zinc-600 dark:text-zinc-400 line-clamp-2">
                                        {{ wo.description }}
                                    </p>
                                    <p v-else class="text-sm text-zinc-400 italic">No description</p>
                                ...
                                <div class="flex flex-col gap-1.5 shrink-0">
                                    ...
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium border bg-blue-50 text-blue-700 border-blue-200">
                                        {{ wo.type }}
                                    </span>
                                </div>
                            </div>
