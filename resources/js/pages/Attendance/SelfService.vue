...
import { attendancesIndex, selfService as attendancesSelfService } from '@/actions/App/Http/Controllers/AttendanceController'
  ...
  try {
    ...
    setTimeout(() => {
      ...
      thisWeekAttendance.value = [
        { date: '2026-03-01', checkIn: '07:58', checkOut: '17:02', status: 'present' },
        { date: '2026-03-02', checkIn: '08:05', checkOut: '17:00', status: 'present' },
        { date: '2026-03-03', checkIn: '07:45', checkOut: '17:15', status: 'present' },
        { date: '2026-03-04', checkIn: null, checkOut: null, status: 'absent' },
        { date: '2026-03-05', checkIn: '07:50', checkOut: null, status: 'present' },
      ]
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-col lg:flex-row gap-6 p-4 lg:p-6 max-w-7xl mx-auto w-full">
      ...
      <div class="flex-1 space-y-4 lg:space-y-6">
        ...
        <!-- Header Section -->
        <div class="bg-gradient-to-br from-blue-600 to-blue-700 dark:from-blue-900 dark:to-blue-800 rounded-2xl p-6 text-white shadow-lg">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <p class="text-blue-100 text-sm">Selamat datang,</p>
              <h1 class="text-2xl lg:text-3xl font-bold mt-1">{{ user?.name }}</h1>
              <div class="mt-2 flex flex-wrap items-center gap-2 text-sm">
                <span class="bg-white/20 px-3 py-1 rounded-full">{{ user?.nik }}</span>
                <span class="bg-white/20 px-3 py-1 rounded-full">{{ user?.position }}</span>
              </div>
            </div>
            <Link
              :href="logout.url()"
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800 overflow-hidden">
          <div class="p-4 lg:p-6 border-b border-zinc-200 dark:border-zinc-800">
            ...
            <p class="text-sm text-zinc-500 mt-1">{{ new Date().toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) }}</p>
          </div>
            ...
            <div v-else class="space-y-4">
              <div class="grid grid-cols-2 gap-4">
                <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4">
                  ...
                  <p class="text-xl font-bold text-green-600">{{ todayAttendance.checkIn }}</p>
                </div>
                <div v-if="todayAttendance.checkOut" class="bg-gray-50 dark:bg-gray-900/20 rounded-xl p-4">
                  <p class="text-sm text-zinc-500 mb-1">Check-Out</p>
                  <p class="text-xl font-bold text-gray-600">{{ todayAttendance.checkOut }}</p>
                </div>
                <div v-else class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
                  <p class="text-sm text-zinc-500 mb-1">Status</p>
                  <p class="text-xl font-bold text-blue-600">Aktif</p>
                </div>
              </div>

              <div v-if="todayAttendance.location" class="bg-zinc-50 dark:bg-zinc-800 rounded-xl p-3 flex items-center gap-2 text-sm">
                <MapPin class="w-4 h-4 text-zinc-500" />
                <span class="text-zinc-600 dark:text-zinc-400">{{ todayAttendance.location }}</span>
              </div>
            ...
            >
              <div class="flex-1">
                ...
                <div class="flex items-center gap-4 mt-1 text-sm text-zinc-500">
                  <span v-if="attendance.checkIn" class="flex items-center gap-1">
                    <Clock class="w-3 h-3" />
              ...
              <span
                :class="['px-3 py-1 rounded-full text-xs font-medium capitalize', getStatusBadge(attendance.status)]"
              >
