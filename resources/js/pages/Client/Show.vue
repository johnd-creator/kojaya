...
import { ArrowLeft, Building2, User, Mail, Phone, MapPin, FileText, Calendar } from 'lucide-vue-next';
...
const projectStatusColors: Record<string, string> = {
    ...
    ON_HOLD: 'bg-zinc-100 text-zinc-700 dark:bg-zinc-900/30 dark:text-zinc-400',
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">{{ client.name }}</h1>
                ...
                <CardContent class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <span class="ml-2 px-2 py-0.5 text-xs font-medium rounded-full" :class="clientTypeColors[client.client_type]">
                                {{ client.client_type }}
                            </span>
                        </div>
                        <div v-if="client.organization">
                            <span class="text-sm font-medium text-zinc-500">Organization</span>
                            <span class="ml-2 text-sm text-zinc-900 dark:text-white">{{ client.organization.name }}</span>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex items-start gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-zinc-900 dark:text-white">{{ client.contact_person }}</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-zinc-900 dark:text-white">{{ client.email }}</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-zinc-900 dark:text-white">{{ client.phone }}</p>
                            </div>
                        </div>
                        <div v-if="client.tax_id" class="flex items-start gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-zinc-900 dark:text-white">{{ client.tax_id }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="client.address" class="flex items-start gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-zinc-900 dark:text-white whitespace-pre-line">{{ client.address }}</p>
                        </div>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card>
                ...
                <CardContent>
                        ...
                        >
                            <div class="flex-1">
                                <Link :href="`/projects/${project.id}`" class="block">
                                    ...
                                    <div class="text-sm text-zinc-500">{{ project.project_code }}</div>
                                </Link>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="text-right">
                                    <div class="text-sm font-medium text-zinc-900 dark:text-white">{{ formatCurrency(project.budget) }}</div>
                                    <div class="text-xs text-zinc-500">Budget</div>
                                </div>
                                <span class="px-2 py-0.5 text-xs font-medium rounded-full" :class="projectStatusColors[project.status]">
                                    {{ project.status }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div v-else class="text-center py-8">
                        <p class="text-sm text-zinc-500">No projects found for this client.</p>
                        <Link :href="`/projects/create?client_id=${client.id}`">
