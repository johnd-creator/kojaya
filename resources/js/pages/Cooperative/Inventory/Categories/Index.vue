...
import { ref } from 'vue';
import { destroy, index, store, update } from '@/routes/cooperative/pos-categories';
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ title: 'Inventory POS', href: '#' }, { title: 'Kategori', href: index().url }]">
        <div class="mx-auto grid w-full max-w-6xl gap-6 p-6 lg:grid-cols-[360px_1fr]">
            <form class="rounded-lg border bg-white p-4 dark:bg-zinc-900" @submit.prevent="submit">
                <div class="flex items-center gap-2">
                    ...
                    <h1 class="text-xl font-semibold">{{ editingId ? 'Edit Kategori' : 'Kategori Baru' }}</h1>
                </div>
                        ...
                        <tr v-for="category in props.categories" :key="category.id">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    <Button size="sm" variant="outline" @click="edit(category)">Edit</Button>
                                    <Button size="sm" variant="outline" @click="router.delete(destroy(category.id).url)">
