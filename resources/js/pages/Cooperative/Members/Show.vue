...
defineProps<{ member: any; openingSavingBalance: number | string | null }>();
</script>
    ...
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-6 p-6">
      <div>
        ...
        <p class="text-sm text-zinc-500">
          {{ member.member_no }} · {{ member.status }}
        </p>
      </div>
      <div class="grid gap-4 md:grid-cols-3">
        ...
        <div class="rounded-lg border bg-white p-4 dark:bg-zinc-900">
          <div class="text-sm text-zinc-500">Akun User</div>
          <div class="font-medium">
            {{ member.user ? member.user.email : "Belum tertaut" }}
          </div>
        </div>
        <div class="rounded-lg border bg-white p-4 dark:bg-zinc-900">
          <div class="text-sm text-zinc-500">Saldo Awal</div>
          <div class="font-medium">
            {{ formatCurrency(openingSavingBalance ?? 0) }}
          </div>
        </div>
        <div class="rounded-lg border bg-white p-4 dark:bg-zinc-900">
          <div class="text-sm text-zinc-500">Total Simpanan</div>
          <div class="font-medium">
            {{ formatCurrency(member.saving_balance ?? 0) }}
          </div>
        </div>
      </div>
      <div class="rounded-lg border bg-white p-4 dark:bg-zinc-900">
