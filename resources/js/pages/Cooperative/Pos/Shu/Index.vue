...
const allocations = computed(() => ((isClosed.value ? props.closedPeriod?.allocations : props.preview?.allocations) ?? []).filter((allocation: any) => Number(allocation.pos_points) > 0));
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ title: 'POS Toko', href: '#' }, { title: 'SHU POS Tahunan', href: index().url }]">
        <div class="mx-auto flex w-full max-w-7xl flex-col gap-6 p-6">
            <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-zinc-500">Tambahan SHU dari profit toko koperasi, dibagi berdasarkan poin profit transaksi anggota.</p>
                </div>
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div class="rounded-lg border bg-white p-5 dark:bg-zinc-900">
                    ...
                    <div class="mt-2 text-2xl font-semibold">{{ formatCurrency(totals.pos_profit_pool) }}</div>
                </div>
                <div class="rounded-lg border bg-white p-5 dark:bg-zinc-900">
                    <div class="text-sm text-zinc-500">Total Poin POS</div>
                    <div class="mt-2 text-2xl font-semibold">{{ totals.total_pos_points }}</div>
                </div>
                <div class="rounded-lg border bg-white p-5 dark:bg-zinc-900">
                    <div class="text-sm text-zinc-500">Anggota Berpoin</div>
                    <div class="mt-2 text-2xl font-semibold">{{ allocations.length }}</div>
                </div>
                <div class="rounded-lg border bg-white p-5 dark:bg-zinc-900">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="allocation in allocations" :key="allocation.member.id">
                            ...
                            <td class="px-4 py-3 text-right font-medium">{{ formatCurrency(allocation.pos_shu_amount) }}</td>
                        </tr>
                        <tr v-if="allocations.length === 0">
                            <td colspan="3" class="px-4 py-10 text-center text-zinc-500">Belum ada transaksi anggota yang menghasilkan poin POS pada tahun ini.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
