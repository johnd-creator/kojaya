...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: `${props.employee.first_name} ${props.employee.last_name}`, href: '#' },
...
const urgencyClass = (days: number | null): string => {
    if (days === null || days < 0) return 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400';
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <Button variant="outline" size="icon" as-child>
                        <Link :href="employeesIndex().url"><ArrowLeft class="h-4 w-4" /></Link>
                    </Button>
                    <div>
                        <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">
                            {{ employee.first_name }} {{ employee.last_name }}
                        </h1>
                        <p class="text-sm text-zinc-500">{{ employee.employee_code }} · {{ employee.organization?.name }}</p>
                    </div>
                </div>
                    ...
                    <DialogContent class="sm:max-w-md">
                        ...
                        <form @submit.prevent="submitContract" class="space-y-4 mt-2">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="grid gap-2">
                                    <Label>End Date <span class="text-zinc-400">(if PKWT)</span></Label>
                            ...
                            <div class="flex justify-end gap-2 pt-2">
                                ...
                                <Button type="submit" :disabled="form.processing">Save Contract</Button>
                            </div>
                        </form>
                    </DialogContent>
                </Dialog>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                            <tr v-for="contract in contracts" :key="contract.id" class="hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                                <td class="px-5 py-4">
                                    <span class="font-semibold text-zinc-800 dark:text-white">{{ contract.type }}</span>
                                </td>
                                <td class="px-5 py-4 text-zinc-600 dark:text-zinc-300">{{ new Date(contract.start_date).toLocaleDateString() }}</td>
                                <td class="px-5 py-4 text-zinc-600 dark:text-zinc-300">
                                    {{ contract.end_date ? new Date(contract.end_date).toLocaleDateString() : '—' }}
                                </td>
                                <td class="px-5 py-4">
                                    <template v-if="contract.end_date">
                                        <span class="px-2.5 py-1 text-xs font-medium rounded-full flex items-center gap-1.5 w-fit"
                                            :class="urgencyClass(daysRemaining(contract.end_date))">
                                            <AlertTriangle v-if="daysRemaining(contract.end_date) !== null && daysRemaining(contract.end_date)! <= 30" class="h-3 w-3" />
                                            {{ daysRemaining(contract.end_date) !== null ? `${daysRemaining(contract.end_date)} days` : 'Expired' }}
                                        </span>
                                    </template>
                                ...
                                <td class="px-5 py-4">
                                    <span class="px-2.5 py-1 text-xs font-medium rounded-full"
                                        :class="{
                                            ...
                                            'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400': contract.status === 'EXPIRED' || contract.status === 'TERMINATED',
                                        }">
                                        {{ contract.status }}
                                    </span>
                                </td>
                            ...
                            <tr v-if="!contracts.length">
                                <td colspan="5" class="py-12 text-center text-zinc-500">
                                    <FileText class="h-12 w-12 mx-auto text-zinc-300 dark:text-zinc-700 mb-3" />
                                    <p class="font-medium text-zinc-800 dark:text-zinc-200">No contracts yet</p>
                                    <p class="text-sm mt-1">Add the first employment contract for this employee.</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
