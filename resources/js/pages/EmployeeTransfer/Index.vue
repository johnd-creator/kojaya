...
watch([statusFilter, orgFilter], () => {
  clearTimeout(t);
  t = setTimeout(() => {
    router.get("/employee-transfers", params.value, {
      preserveState: true,
      replace: true,
    });
  }, 250);
    ...
    <div class="flex flex-1 flex-col gap-6 p-6 max-w-6xl mx-auto w-full">
      ...
      <div class="flex flex-wrap gap-3">
        <select
          ...
          class="flex h-10 w-full max-w-[220px] rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950"
        >
        ...
        <select
          v-model="orgFilter"
          class="flex h-10 w-full max-w-[320px] rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950"
        >
      ...
      >
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
            <tr
              v-for="tfr in transfers.data"
              :key="tfr.id"
              class="hover:bg-zinc-50 dark:hover:bg-zinc-800/30"
            >
              <td class="px-5 py-4 text-zinc-900 dark:text-white">
                {{ tfr.employee?.employee_code }} -
                {{ tfr.employee?.first_name }} {{ tfr.employee?.last_name }}
              </td>
              <td class="px-5 py-4 text-zinc-500">
                {{ tfr.from_organization?.name ?? "-" }}
              </td>
              <td class="px-5 py-4 text-zinc-500">
                {{ tfr.to_organization?.name ?? "-" }}
              </td>
