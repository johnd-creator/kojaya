...
const submitCreate = () => {
  router.post("/finance/bank-batches", form.value, {
    onSuccess: () => {
...
const submitReconcile = () => {
  router.post("/finance/bank-batches/reconcile", {
    statement_csv: statementCsv.value,
  });
};
</script>
...
<template>
  ...
  >
    <div class="flex flex-1 flex-col gap-6 p-6 w-full">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-zinc-500 mt-1">
            Mass transfer and auto-reconciliation
          </p>
        </div>
        ...
        <div class="lg:col-span-2">
          ...
          >
            <table class="w-full text-left">
              <thead
                class="bg-zinc-50 dark:bg-zinc-900/50 text-zinc-500 text-xs"
              >
                <tr>
              ...
              <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                ...
                <tr v-for="b in props.batches" :key="b.id">
                  ...
                  <td class="px-6 py-3">
                    {{
                      new Date(b.batch_date).toLocaleDateString("id-ID", {
                        day: "numeric",
                        month: "long",
                        year: "numeric",
                      })
                  ...
                  <td class="px-6 py-3">
                    <span
                      class="inline-flex items-center px-2 py-0.5 text-xs rounded border"
                      >{{ b.status }}</span
                    >
                  </td>
                  <td class="px-6 py-3 text-right">
                    <Button variant="outline" as-child>
                      <a
                        :href="`/finance/bank-batches/${b.id}/export`"
                        target="_blank"
                      >
        ...
        <div>
          ...
          >
            <h2 class="text-lg font-semibold mb-4 flex items-center gap-2">
              <UploadCloud class="h-5 w-5" /> Reconcile Statement
            </h2>
            <div class="space-y-2">
              <Label>Paste Statement CSV</Label>
              <textarea
                v-model="statementCsv"
                rows="8"
                class="w-full rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-3 py-2 text-sm"
              ></textarea>
            </div>
            <div class="mt-3">
      ...
      >
        <div
          class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-lg w-full max-w-2xl p-6"
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="space-y-2">
              ...
              <select
                v-model="form.format"
                class="h-10 rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-3 text-sm"
              >
          ...
          <div class="mt-6 flex items-center gap-2">
            <Button class="bg-indigo-600 text-white" @click="submitCreate"
              >Create Batch</Button
            >
