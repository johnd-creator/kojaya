...
defineProps<{
    summary: { savings_balance: number; total_paid: number; pending_invoices: number };
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ title: 'Kojayaku', href: '/member' }, { title: 'Simpanan', href: '/member/savings' }]">
        <PageContainer>
            <div class="grid gap-4 md:grid-cols-3">
                <div class="rounded-lg border p-4"><div class="text-sm text-muted-foreground">Saldo Simpanan</div><div class="mt-2 text-2xl font-semibold">{{ formatCurrency(summary.savings_balance) }}</div></div>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody>
                            <tr v-for="entry in entries.data" :key="entry.id" class="border-t">
                                ...
                                <td class="px-4 py-3 text-right">{{ formatCurrency(entry.debit) }}</td>
                                <td class="px-4 py-3 text-right">{{ formatCurrency(entry.credit) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                        ...
                        <table class="w-full text-left text-sm">
                            ...
                            <tbody>
                                <tr v-for="invoice in invoices.data" :key="invoice.id" class="border-t">
                                    ...
                                    <td class="px-4 py-3">{{ invoice.contribution_type?.name || '-' }}</td>
