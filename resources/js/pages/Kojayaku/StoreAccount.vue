...
<template>
  ...
  <AppLayout
    :breadcrumbs="[
      ...
      { title: 'Saldo Toko', href: storeAccountRoute().url },
    ]"
  >
    <PageContainer>
      <div class="flex flex-col gap-6">
        <header class="flex items-center gap-3 sm:gap-5">
          ...
          <div class="flex-1">
            ...
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
              Pantau saldo, limit, dan riwayat transaksi belanja Anda.
            </p>
          </div>
        </header>
        ...
        >
          ...
          >
            <p class="text-xs font-bold uppercase tracking-wider text-zinc-500">
              Sisa yang dapat digunakan
            </p>
            <p class="mt-2 text-2xl font-black text-sky-700 dark:text-sky-300">
              {{ formatCurrency(account.available_spending) }}
            </p>
          </div>
          <div
            class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"
          >
            ...
            <Badge class="mt-3" :variant="statusVariant(account.status)">{{
        ...
        >
          <Wallet class="mx-auto size-10 text-zinc-500" />
          <h2 class="mt-3 text-lg font-bold text-zinc-900 dark:text-white">
            Akun Saldo Toko belum tersedia
          </h2>
          <p class="mx-auto mt-1 max-w-lg text-sm text-zinc-500">
            Hubungi koperasi untuk mengaktifkan akun Saldo Toko Anda.
          </p>
        </section>
        ...
        >
          <div class="border-b border-zinc-200 p-5 dark:border-zinc-800">
            ...
            <p class="mt-1 text-sm text-zinc-500">
              Nama pembeli, kasir, catatan, nominal, dan waktu tercatat apa
              adanya.
            </p>
          </div>
          <div v-if="ledger.data.length === 0" class="p-10 text-center">
            <RefreshCw
              class="mx-auto size-8 text-zinc-300 dark:text-zinc-700"
            />
            <p
              class="mt-3 text-sm font-semibold text-zinc-600 dark:text-zinc-300"
            >
          ...
          <div v-else class="overflow-x-auto">
            <table class="w-full min-w-[860px] text-sm">
              <thead
                class="bg-zinc-50 text-left text-xs font-bold uppercase tracking-wider text-zinc-500 dark:bg-zinc-950/60"
              >
                <tr>
              ...
              <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                <tr v-for="entry in ledger.data" :key="entry.id">
                  <td
                    class="whitespace-nowrap px-5 py-4 text-zinc-600 dark:text-zinc-300"
                  >
                  ...
                  <td
                    class="px-5 py-4 font-medium text-zinc-900 dark:text-white"
                  >
                  ...
                  <td
                    class="max-w-[220px] px-5 py-4 text-zinc-600 dark:text-zinc-300"
                  >
                    {{ entry.purchase_note ?? "—" }}
                  </td>
                  <td
                    class="whitespace-nowrap px-5 py-4 text-right font-mono font-semibold"
                    :class="
                      entry.effect === 'credit'
                        ? 'text-emerald-700'
                        : 'text-rose-600'
                  ...
                  <td
                    class="whitespace-nowrap px-5 py-4 text-right font-mono font-semibold text-zinc-900 dark:text-white"
                  >
                    {{ formatCurrency(entry.balance_after) }}
                  </td>
                  <td class="px-5 py-4">
                    <Badge v-if="entry.status" variant="secondary">{{
                      entryLabel(entry)
                    }}</Badge>
                    <Badge v-if="entry.is_reversed" variant="destructive"
                      >Dibatalkan</Badge
                    >
                  </td>
                </tr>
          ...
          <div
            v-if="ledger.links.length > 0"
            class="flex flex-wrap items-center justify-center gap-2 border-t border-zinc-200 p-4 dark:border-zinc-800"
          >
            <Link
              v-for="(link, index) in ledger.links"
              :key="index"
              :href="link.url || '#'"
              :class="[
