        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-6 w-full">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-zinc-500 mt-1.5 ml-10">Review and manage employee time off requests</p>
                </div>

                <div class="flex items-center gap-2">
                    <select v-model="filters.status" @change="router.get('/leaves', { status: ($event.target as HTMLSelectElement).value }, { preserveState: true })" class="flex h-9 rounded-md border border-zinc-200 bg-white px-3 py-1 text-sm text-zinc-900 shadow-sm transition-colors focus-visible:outline-none dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100">
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        <thead class="text-xs text-zinc-500 bg-zinc-50 dark:bg-zinc-800 uppercase border-b border-zinc-200 dark:border-zinc-800">
                            <tr>
                                <th class="px-8 py-4 font-medium text-xs uppercase tracking-wider">Employee</th>
                                <th class="px-8 py-4 font-medium text-xs uppercase tracking-wider">Leave Details</th>
                                <th class="px-8 py-4 font-medium text-xs uppercase tracking-wider">Dates & Duration</th>
                                <th class="px-8 py-4 font-medium text-xs uppercase tracking-wider">Reason</th>
                                <th class="px-8 py-4 font-medium text-xs uppercase tracking-wider text-center">Attachment</th>
                                <th class="px-8 py-4 font-medium text-xs uppercase tracking-wider text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                            <tr v-for="leave in leaves.data" :key="leave.id" class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                <td class="px-8 py-5">
                                    ...
                                    <div class="text-xs text-zinc-500 mt-0.5">{{ leave.employee?.employee_code }}</div>
                                </td>
                                <td class="px-8 py-5">
                                ...
                                <td class="px-8 py-5 text-zinc-600 dark:text-zinc-400">
                                    <div class="font-medium text-zinc-900 dark:text-zinc-200 whitespace-nowrap">{{ formatTanggal(leave.start_date) }} &mdash; {{ formatTanggal(leave.end_date) }}</div>
                                    <div class="text-xs mt-0.5">{{ formatDurasi(leave.total_days) }}</div>
                                </td>
                                <td class="px-8 py-5">
                                    <p class="text-sm text-zinc-600 dark:text-zinc-400 line-clamp-2 max-w-xs" :title="leave.reason">
                                        {{ leave.reason }}
                                    </p>
                                </td>
