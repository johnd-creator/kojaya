...
import { show } from "@/routes/petty-cash";
...
const confirmDeleteAccount = (): void => {
  ...
  router.delete(destroy({ petty_cash: deleteTarget.value.id }).url, {
    onFinish: () => {
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
              ...
              >
                <td class="py-4 px-6">
                  <div class="flex items-center gap-3">
                    <div
                      class="h-10 w-10 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center shrink-0"
                    >
                      <Wallet
                    ...
                    <div>
                      <Link
                        :href="show({ petty_cash: account.id }).url"
                        class="font-medium text-zinc-900 dark:text-zinc-100 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                      >
                        {{ account.name }}
                      </Link>
                      <p
                        class="text-xs text-zinc-500 mt-0.5 truncate max-w-[200px]"
                      >
                        {{ account.description }}
                      </p>
                    </div>
                ...
                <td class="py-4 px-6 text-right">
                  ...
                  >
                    <Link :href="show({ petty_cash: account.id }).url">
                      <Button
                        variant="ghost"
                        size="icon"
    ...
    <ConfirmDialog
      ...
      variant="danger"
      title="Hapus akun petty cash"
      :message="`Hapus akun ${deleteTarget?.name ?? ''}?`"
      confirm-label="Hapus"
