...
const updateDepartmentId = (value: unknown) => {
  form.department_id = value == null ? "" : String(value);
};

const updateJobGradeId = (value: unknown) => {
  form.job_grade_id = value == null ? "" : String(value);
};

const updateDepartmentFilter = (value: unknown) => {
  filters.value.department_id =
    value === allDepartmentsValue || value == null ? "" : String(value);
};
...
const openEdit = (p: any) => {
  ...
  form.job_grade_id = p.job_grade_id;
  showModal.value = true;
};

const submit = () => {
  if (editTarget.value) {
    form.put(`/positions/${editTarget.value.id}`, {
      onSuccess: () => {
...
const confirmDestroy = () => {
  ...
  router.delete(`/positions/${deleteTarget.value.id}`, {
    onFinish: () => {
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-1 flex-col gap-6 p-6 max-w-6xl mx-auto w-full">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-zinc-500 mt-1">
            Jabatan karyawan per bidang dan jenjang.
          </p>
        </div>
        <Dialog v-model:open="showModal">
          <DialogTrigger as-child>
            <Button @click="openCreate"
              ><Plus class="h-4 w-4 mr-2" />Add Position</Button
            >
          </DialogTrigger>
          <DialogContent class="sm:max-w-md">
            <DialogHeader
              ><DialogTitle
                >{{ editTarget ? "Edit" : "New" }} Position</DialogTitle
              ></DialogHeader
            >
            <form @submit.prevent="submit" class="space-y-4 mt-2">
              <div class="grid grid-cols-2 gap-4">
                <div class="grid gap-2">
                  <Label>Code</Label
                  ><Input v-model="form.code" placeholder="OCR_A" required />
                </div>
                <div class="grid gap-2">
                  <Label>Job Grade</Label>
                  <Select
                    :model-value="form.job_grade_id"
                    @update:model-value="updateJobGradeId"
                  >
              ...
              <div class="grid gap-2">
                ...
                ><Input
                  v-model="form.name"
                  placeholder="Operator Control Room A"
                  required
                />
              </div>
              <div class="grid gap-2">
                <Label>Department / Bidang</Label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem
                      ...
                      :key="dept.id"
                      :value="dept.id"
                      >{{ dept.code }} — {{ dept.name }}</SelectItem
                    >
                  </SelectContent>
                </Select>
              </div>
      ...
      >
        <Select
          :model-value="filters.department_id || allDepartmentsValue"
          @update:model-value="updateDepartmentFilter"
        >
          <SelectTrigger class="w-full sm:w-[220px]">
            <SelectValue placeholder="All Departments" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem :value="allDepartmentsValue"
      ...
      >
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
            <tr
              v-for="p in positions.data"
              :key="p.id"
              class="hover:bg-zinc-50 dark:hover:bg-zinc-800/30"
            >
              <td
                class="px-5 py-4 font-mono text-indigo-600 dark:text-indigo-400 text-xs"
              >
                {{ p.code }}
              </td>
              ...
              <td class="px-5 py-4">
                <span
                  class="px-2 py-0.5 text-xs rounded-full bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30"
                  >{{ p.job_grade?.name }}</span
                >
              </td>
            ...
            <tr v-if="!positions.data.length">
              <td colspan="6" class="py-12 text-center text-zinc-500">
                <Briefcase class="h-12 w-12 mx-auto text-zinc-300 mb-3" />
      ...
      <ConfirmDialog
        ...
        :message="`Hapus position ${deleteTarget?.name ?? ''}?`"
        confirm-label="Hapus"
