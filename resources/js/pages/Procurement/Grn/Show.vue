...
const form = useForm({
  items: props.poItems.map((it) => ({ po_item_id: it.id, received_qty: 0, condition: 'Good' })),
})
...
function receiveAll() {
  form.items.forEach(formItem => {
    const originalItem = itemsWithRemaining.value.find(i => i.id === formItem.po_item_id)
    if (originalItem) {
  ...
  <AppLayout>
    <div class="p-6 max-w-5xl mx-auto space-y-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <div class="text-sm text-zinc-500 mt-1">
            ...
            Status: <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-zinc-100 text-zinc-700">{{ props.grn.status }}</span>
          </div>
        </div>
      </div>

      <div v-if="props.grn.status !== 'RECEIVED_FULL'" class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6 space-y-6">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-medium">Receive Items</h2>
          <button type="button" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium" @click="receiveAll">
            Auto-fill Remaining
          </button>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
              <tr v-for="(formItem, i) in form.items" :key="formItem.po_item_id">
                <td class="px-4 py-3 font-medium">
                  {{ itemsWithRemaining.find(x => x.id === formItem.po_item_id)?.description }}
                </td>
                <td class="px-4 py-3 text-right text-zinc-500">
                  {{ itemsWithRemaining.find(x => x.id === formItem.po_item_id)?.qty }}
                </td>
                <td class="px-4 py-3 text-right text-zinc-500">
                  {{ itemsWithRemaining.find(x => x.id === formItem.po_item_id)?.received_total }}
                </td>
                <td class="px-4 py-3 text-right font-medium" :class="{'text-emerald-600': itemsWithRemaining.find(x => x.id === formItem.po_item_id)?.remaining === 0}">
                  {{ itemsWithRemaining.find(x => x.id === formItem.po_item_id)?.remaining }}
                </td>
                <td class="px-4 py-3">
                  <input 
                    type="number" 
                    ...
                    min="0"
                    :max="itemsWithRemaining.find(x => x.id === formItem.po_item_id)?.remaining"
                    :disabled="itemsWithRemaining.find(x => x.id === formItem.po_item_id)?.remaining === 0"
                  />
                </td>
                <td class="px-4 py-3">
                  <select 
      ...
      <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
        <h2 class="text-lg font-medium mb-4">Receipt History (All GRNs for this PO)</h2>
        <div v-if="props.allReceivedItems.length === 0" class="text-sm text-zinc-500 text-center py-8 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg border border-dashed border-zinc-200 dark:border-zinc-700">
          ...
          <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
              <tr v-for="it in props.allReceivedItems" :key="it.id">
                <td class="px-4 py-3 font-medium">
                  {{ props.poItems.find(p => p.id === it.purchase_order_item_id)?.description }}
                </td>
                <td class="px-4 py-3 text-right font-medium">{{ it.received_qty }}</td>
                <td class="px-4 py-3">
                  <span :class="{
                    'text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full text-xs': it.condition === 'Good',
                    'text-red-600 bg-red-50 px-2 py-0.5 rounded-full text-xs': it.condition !== 'Good'
                  }">
                    {{ it.condition }}
                  </span>
                </td>
                <td class="px-4 py-3 text-right text-zinc-500">{{ new Date(it.created_at).toLocaleDateString() }}</td>
              </tr>
            </tbody>
          </table>
        </div>
