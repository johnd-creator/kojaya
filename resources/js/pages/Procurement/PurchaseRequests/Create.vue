      ...
      >
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              v-model="form.title"
              class="w-full border border-zinc-300 dark:border-zinc-700 rounded-md px-3 py-2 bg-transparent focus:ring-2 focus:ring-indigo-500 focus:outline-none"
              placeholder="e.g. Office Supplies Q1"
            />
            <div class="text-xs text-red-500 mt-1" v-if="form.errors.title">
              {{ form.errors.title }}
            </div>
          </div>
          <div>
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            ...
            <button
              type="button"
              class="text-sm px-3 py-1.5 rounded-md bg-indigo-50 text-indigo-700 hover:bg-indigo-100 font-medium transition-colors"
              @click="addItem"
            >
              + Add Item
            </button>
          ...
          >
            <table class="w-full text-sm text-left">
                ...
                >
                  <td class="px-4 py-2 space-y-2">
                    ...
                    <input
                      ...
                      class="w-full border-0 bg-transparent focus:ring-0 p-0 placeholder-zinc-400 text-sm"
                    />
                  </td>
                  <td class="px-4 py-2 align-top">
                    <select
                      v-model="it.gl_account"
                      class="w-full border-0 bg-transparent focus:ring-0 p-0 text-zinc-600 dark:text-zinc-300"
                    >
                      <option
                        v-for="gl in props.glAccounts"
                        :key="gl"
                        :value="gl"
                      >
                        {{ gl }}
                  ...
                  <td class="px-4 py-2 align-top">
                    <input
                      ...
                      class="w-full border-0 bg-transparent focus:ring-0 p-0 text-right"
                      min="1"
                    />
                  </td>
                  <td class="px-4 py-2 align-top">
                  ...
                  <td
                    class="px-4 py-2 text-right font-medium text-zinc-700 dark:text-zinc-300 align-top"
                  >
              ...
              <tfoot
                class="bg-zinc-50 dark:bg-zinc-800/50 border-t border-zinc-200 dark:border-zinc-800 font-semibold"
              >
                <tr>
                  <td colspan="4" class="px-4 py-3 text-right">Grand Total</td>
                  <td
                    class="px-4 py-3 text-right text-indigo-600 dark:text-indigo-400"
                  >
          ...
          <div class="text-xs text-red-500 mt-1" v-if="form.errors['items']">
            {{ form.errors["items"] }}
          </div>
        </div>
