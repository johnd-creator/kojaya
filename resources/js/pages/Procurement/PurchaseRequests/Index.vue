...
const columns = [
  ...
  { header: 'Submitted At', key: 'submitted_at', format: (v: string) => v ? new Date(v).toLocaleDateString() : '-', align: 'right' },
]

const stats = computed(() => {
  return {
    total: props.requests.length,
    pending: props.requests.filter(r => ['SUBMITTED', 'APPROVAL_L1', 'APPROVAL_L2', 'APPROVAL_L3'].includes(r.status)).length,
    approved: props.requests.filter(r => ['APPROVED', 'PO_CREATED'].includes(r.status)).length
  }
})
  ...
  <AppLayout>
    <div class="flex flex-1 flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 class="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">Purchase Requests</h1>
          <p class="text-zinc-500 mt-1">Manage purchase requests and approvals.</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h2 class="text-3xl font-bold text-zinc-900 dark:text-white mt-1">{{ stats.total }}</h2>
          </div>
        ...
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h2 class="text-3xl font-bold text-zinc-900 dark:text-white mt-1">{{ stats.pending }}</h2>
          </div>
        ...
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h2 class="text-3xl font-bold text-zinc-900 dark:text-white mt-1">{{ stats.approved }}</h2>
          </div>
          <div class="h-12 w-12 rounded-full bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
