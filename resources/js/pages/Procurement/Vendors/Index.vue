  ...
  <AppLayout>
    <div class="flex flex-1 flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">Vendors</h1>
          <p class="text-zinc-500 mt-1">Manage suppliers and vendor information.</p>
        </div>
        <div>
          <!-- <Button as-child>
      ...
      >
        <template #email="{ value }">
          <a v-if="value" :href="`mailto:${value}`" class="text-indigo-600 hover:underline" @click.stop>{{ value }}</a>
          <span v-else class="text-zinc-400">-</span>
        </template>

        <template #status="{ value }">
          <StatusBadge :status="value ? 'ACTIVE' : 'INACTIVE'" :variant="value ? 'success' : 'secondary'" />
        </template>
