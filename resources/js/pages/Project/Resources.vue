...
const pendingAllocation = ref<any>(null);
...
const submitEdit = () => {
  editForm.put(
    `/projects/${props.project.id}/resources/assets/${selectedAllocation.value.id}`,
    {
      onSuccess: () => {
...
const deleteAllocation = (allocation: any) => {
  pendingAllocation.value = allocation;
  deleteDialogOpen.value = true;
};

const confirmDeleteAllocation = (): void => {
  if (!pendingAllocation.value) {
    return;
  }

  useForm({}).delete(
    `/projects/${props.project.id}/resources/assets/${pendingAllocation.value.id}`,
    {
      onFinish: () => {
...
const getStatusColor = (status: string) => {
  switch (status) {
    ...
    default:
      return "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300"; // planned
  }
};
</script>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-zinc-500 mt-1">
            Manage heavy equipment and assets for {{ project.name }}
          </p>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
              ...
              <tr
                v-for="alloc in assetAllocations"
                :key="alloc.id"
              ...
              >
                ...
                <td class="px-6 py-4">
                  <span
                    ...
                    :class="getStatusColor(alloc.status)"
                  >
                    {{ alloc.status }}
                  </span>
                </td>
        ...
        <DialogContent class="sm:max-w-md">
          ...
          <form @submit.prevent="submitCreate" class="space-y-4 mt-2">
            <div class="grid gap-2">
              ...
              <span v-if="form.errors.asset_id" class="text-xs text-red-500">{{
            ...
            <div class="grid gap-2">
              ...
              <Input
                v-model="form.notes"
                placeholder="e.g. Needs transport trailer"
              />
            </div>
      ...
      <ConfirmDialog
        ...
        variant="danger"
        title="Hapus alokasi aset"
        message="Apakah Anda yakin ingin menghapus alokasi aset ini dari proyek?"
        confirm-label="Hapus"
        @confirm="confirmDeleteAllocation"
      />
    </div>
  </AppLayout>
</template>
