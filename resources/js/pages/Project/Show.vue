...
const GanttChart = defineAsyncComponent(
  () => import("@/components/project/GanttChart.vue"),
);
...
const pendingTeamMemberId = ref<string | null>(null);
const pendingDocumentId = ref<string | null>(null);
const availabilityLoading = ref(false);
const teamAvailability = ref<{ available: boolean; conflicts: any[] } | null>(
  null,
);
...
const removeTeamMember = (teamMemberId: string) => {
  pendingTeamMemberId.value = teamMemberId;
  removeTeamDialogOpen.value = true;
};

const confirmRemoveTeamMember = (): void => {
  if (!pendingTeamMemberId.value) {
    return;
  }

  router.delete(
    `/projects/${props.project.id}/team/${pendingTeamMemberId.value}`,
    {
      onFinish: () => {
        removeTeamDialogOpen.value = false;
        pendingTeamMemberId.value = null;
      },
    },
  );
};
...
const confirmDeleteDocument = (): void => {
  if (!pendingDocumentId.value) {
    return;
  }

  router.delete(
    `/projects/${props.project.id}/documents/${pendingDocumentId.value}`,
    {
      onFinish: () => {
...
const submitMobilization = () => {
  ...
  mobilizationForm.post(
    `/projects/${props.project.id}/team/${selectedTeamMember.value.id}/mobilization`,
    {
      onSuccess: () => {
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-col">
      <!-- Header -->
      <div
        class="border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-6 py-4"
      >
        <div
              ...
              <div class="space-y-4">
                <div class="flex justify-between items-center">
                  <div class="flex items-center gap-2">
                    ...
                    <div
                      class="flex items-center rounded-md border border-zinc-200 dark:border-zinc-800 p-1"
                    >
                      <Button
                        variant="ghost"
                        size="sm"
                        class="h-7 px-2"
                        :class="{
                          'bg-zinc-100 dark:bg-zinc-800': viewMode === 'list',
                        }"
                        @click="viewMode = 'list'"
                      >
                        List
                      ...
                      <Button
                        ...
                        class="h-7 px-2"
                        :class="{
                          'bg-zinc-100 dark:bg-zinc-800': viewMode === 'gantt',
                        }"
                ...
                <div v-if="viewMode === 'list'">
                  <div
                    v-if="rootTasks && rootTasks.length > 0"
                    class="space-y-2"
                  >
                    <div
                      v-for="task in rootTasks"
                      :key="task.id"
                      class="p-4 border rounded-lg bg-white dark:bg-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-800/50"
                    >
                      <div class="flex items-start justify-between">
                        <div class="flex-1">
                          ...
                          <div
                            class="flex items-center gap-4 mt-2 text-xs text-zinc-500"
                          >
                            <span v-if="task.assignee"
                              >Assigned to: {{ task.assignee.first_name }}
                              {{ task.assignee.last_name }}</span
                            >
                        ...
                        <div class="flex items-center gap-2">
                          <div class="text-right">
                            ...
                            <div
                              class="h-2 w-20 bg-zinc-200 rounded-full overflow-hidden"
                            >
                              <div
                  ...
                  <div
                    v-else
                    class="p-8 text-center border rounded-lg bg-white dark:bg-zinc-900"
                  >
                ...
                <div v-else-if="viewMode === 'gantt'">
                  <Suspense>
                    ...
                    <template #fallback>
                      <div
                        class="flex h-[600px] items-center justify-center rounded-lg border bg-white text-sm text-zinc-500 dark:bg-zinc-900 dark:text-zinc-400"
                      >
                        Loading Gantt chart...
                      </div>
                    </template>
                ...
                <DialogContent class="max-w-2xl">
                  ...
                  <form @submit.prevent="submitTask" class="space-y-4">
                    <div class="grid gap-2">
                      ...
                      />
                      <span
                        v-if="taskForm.errors.name"
                        class="text-xs text-red-500"
                        >{{ taskForm.errors.name }}</span
                      >
                    </div>
                    ...
                    <div class="grid gap-2">
                      ...
                      <span
                        ...
                        class="text-xs text-red-500"
                        >{{ taskForm.errors.description }}</span
                      >
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      <div class="grid gap-2">
                        ...
                        <span
                          ...
                          class="text-xs text-red-500"
                          >{{ taskForm.errors.start_date }}</span
                        >
                      </div>
                      ...
                      <div class="grid gap-2">
                        ...
                        />
                        <span
                          v-if="taskForm.errors.end_date"
                          class="text-xs text-red-500"
                          >{{ taskForm.errors.end_date }}</span
                        >
                      </div>
                    </div>
                      ...
                      <div class="grid gap-2">
                        ...
                        >
                          ...
                          >
                            ...
                            {{ member.employee?.last_name }} ({{ member.role }})
                          </option>
                        </select>
                        <span
                          v-if="taskForm.errors.assigned_to"
                          class="text-xs text-red-500"
                          >{{ taskForm.errors.assigned_to }}</span
                        >
                      </div>

                      <div class="grid gap-2">
                        <Label for="task_estimated_hours"
                        ...
                        />
                        <span
                          v-if="taskForm.errors.estimated_hours"
                          class="text-xs text-red-500"
                          >{{ taskForm.errors.estimated_hours }}</span
                        >
                      </div>
                    </div>
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <p class="text-sm text-zinc-500">
                    Track mobilization readiness and detect conflicts
                  </p>
                </div>
                ...
                >
                  <div
                    v-for="member in project.team"
                    :key="member.id"
                    class="flex flex-col gap-3 rounded-lg border bg-white p-4 dark:bg-zinc-900"
                  >
                    <div class="flex items-start justify-between gap-4">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          @click="openMobilizationDialog(member)"
                          >Update</Button
                        >
                      </div>
                    </div>
                    ...
                    <div
                      class="grid grid-cols-2 gap-2 text-sm text-zinc-600 dark:text-zinc-300"
                    >
                      <div>Start: {{ formatDate(member.start_date) }}</div>
                      <div>
                        End:
                        {{
                          member.end_date ? formatDate(member.end_date) : "open"
                        }}
                      </div>
                      <div>PPE: {{ member.has_ppe ? "Yes" : "No" }}</div>
                      <div>
                        Uniform: {{ member.has_uniform ? "Yes" : "No" }}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <DialogContent class="max-w-md">
                  ...
                  <form @submit.prevent="submitMobilization" class="space-y-4">
                    <div class="grid gap-2">
                      <Label>Status</Label>
                      <select
                        v-model="mobilizationForm.status"
                        class="flex h-10 w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950"
                      >
                      ...
                      <span
                        v-if="mobilizationForm.errors.status"
                        class="text-xs text-red-500"
                        >{{ mobilizationForm.errors.status }}</span
                      >
                    </div>

                    <div
                      class="flex items-center justify-between rounded-md border border-zinc-200 p-3 text-sm dark:border-zinc-800"
                    >
                      ...
                      <input
                        type="checkbox"
                        v-model="mobilizationForm.has_ppe"
                        class="h-4 w-4"
                      />
                    </div>
                    ...
                    >
                      ...
                      <input
                        type="checkbox"
                        v-model="mobilizationForm.has_uniform"
                        class="h-4 w-4"
                      />
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                      <Button
                        type="button"
                        variant="outline"
                        @click="showMobilizationDialog = false"
                        >Cancel</Button
                      >
                      <Button
                        type="submit"
                        :disabled="mobilizationForm.processing"
                      >
                        <span v-if="mobilizationForm.processing"
                          >Saving...</span
                        >
              ...
              <div class="space-y-6">
                ...
                >
                  <div
                    v-for="member in project.team"
                    :key="member.id"
                    class="p-4 border rounded-lg bg-white dark:bg-zinc-900"
                  >
                    <div class="flex items-start justify-between">
                      <div class="flex items-center gap-3">
                        <Avatar>
                    ...
                    >
                      <div class="grid grid-cols-2 gap-2 text-xs text-zinc-500">
                        ...
                        <div>
                          ...
                          {{ formatCurrency(member.daily_rate_cost) }}/day
                        </div>
                        <div>
                          <Badge
                            :variant="
                              member.status === 'PLACED'
                                ? 'default'
                                : 'secondary'
                            "
                            >{{ member.status }}</Badge
                ...
                <DialogContent class="max-w-md">
                  ...
                  <form @submit.prevent="submitTeamMember" class="space-y-4">
                    <div class="grid gap-2">
                      ...
                      <span
                        v-if="teamForm.errors.employee_id"
                        class="text-xs text-red-500"
                        >{{ teamForm.errors.employee_id }}</span
                      >
                    </div>
                    ...
                    <div class="grid gap-2">
                      ...
                      />
                      <span
                        v-if="teamForm.errors.role"
                        class="text-xs text-red-500"
                        >{{ teamForm.errors.role }}</span
                      >
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      <div class="grid gap-2">
                        ...
                        />
                        <span
                          v-if="teamForm.errors.start_date"
                          class="text-xs text-red-500"
                          >{{ teamForm.errors.start_date }}</span
                        >
                      </div>
                      ...
                      <div class="grid gap-2">
                        ...
                        />
                        <span
                          v-if="teamForm.errors.end_date"
                          class="text-xs text-red-500"
                          >{{ teamForm.errors.end_date }}</span
                        >
                      </div>
                    </div>
                    ...
                    <div
                      v-else-if="
                        teamAvailability && !teamAvailability.available
                      "
                      class="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-900/10 dark:text-red-200"
                    >
                      <div class="font-medium">Schedule conflict</div>
                      <div class="mt-1 space-y-1">
                        <div
                          v-for="conflict in teamAvailability.conflicts"
                          :key="conflict.project_id"
                        >
                          {{ conflict.project_code }} -
                          {{ conflict.project_name }} ({{
                            conflict.start_date
                          }}
                          - {{ conflict.end_date || "open" }})
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="grid gap-2">
                      ...
                      <Input
                        id="team_daily_rate"
                        type="number"
                        v-model="teamForm.daily_rate_cost"
                        min="0"
                        step="10000"
                        required
                      />
                      <span
                        v-if="teamForm.errors.daily_rate_cost"
                        class="text-xs text-red-500"
                        >{{ teamForm.errors.daily_rate_cost }}</span
                      >
                    </div>
              ...
              <div class="space-y-4">
                ...
                >
                  <div
                    ...
                    :key="doc.id"
                    class="p-4 border rounded-lg bg-white dark:bg-zinc-900 hover:shadow-md transition-shadow"
                  >
                    <div class="flex items-start justify-between">
                      <div class="flex items-center gap-3">
                        <div
                    ...
                    <div class="mt-3 flex items-center justify-between">
                      <Badge
                        :variant="
                          doc.status === 'VALID' ? 'default' : 'destructive'
                        "
                        >{{ doc.status }}</Badge
                      >
                      <a
    ...
    <ConfirmDialog
      ...
      message="Apakah Anda yakin ingin menghapus anggota tim ini dari proyek?"
      confirm-label="Hapus"
    ...
    <ConfirmDialog
      ...
      variant="danger"
      title="Hapus dokumen"
      message="Apakah Anda yakin ingin menghapus dokumen proyek ini?"
      confirm-label="Hapus"
