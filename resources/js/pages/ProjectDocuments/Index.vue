...
import { ArrowLeft, FileText, Trash2, Plus } from "lucide-vue-next";
import { ref } from "vue";
...
const confirmDeleteDocument = (): void => {
  ...
  router.delete(
    `/projects/${props.project.id}/documents/${deleteTargetId.value}`,
    {
      onSuccess: () => {
        router.reload();
      },
            ...
            <form @submit.prevent="submitDocument" class="space-y-4 mt-2">
              ...
              <div class="grid gap-2">
                ...
                <select
                  ...
                  class="w-full border border-zinc-300 dark:border-zinc-700 rounded-md px-3 py-2 bg-white dark:bg-zinc-900"
                >
              ...
              <div class="grid gap-2">
                ...
                <Input
                  ...
                  @input="(e: any) => (documentForm.file = e.target.files[0])"
                  required
                />
              </div>
              <div class="grid gap-2">
      ...
      >
        ...
        >
          <div
            ...
            :key="doc.id"
            class="flex items-center gap-3 p-4 border border-zinc-200 dark:border-zinc-800 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
          >
            <div
            ...
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-1">
                ...
                <span
                  ...
                  :class="documentTypeColors[doc.type]"
                >
                  {{ doc.type }}
                </span>
                <span
                  class="px-2 py-0.5 text-xs font-medium rounded-full"
                  :class="documentStatusColors[doc.status]"
                >
                  {{ doc.status }}
                </span>
              </div>
              <div class="flex items-center gap-2 text-xs text-zinc-500">
                <span v-if="doc.expiry_date"
                  >Expires: {{ doc.expiry_date }}</span
                >
                <span
                  >• {{ new Date(doc.created_at).toLocaleDateString() }}</span
                >
              </div>
            </div>
            <button
