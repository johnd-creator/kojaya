...
import { Users, Plus, Calendar, AlertTriangle, Trash2, Edit2, GripVertical, CheckSquare, Search, ArrowLeft } from 'lucide-vue-next';
...
// Kanban columns
const mobilizationStages = ['RECRUITMENT', 'SCREENING', 'MCU', 'ONBOARDING', 'PLACED'];
...
const deleteMember = (member: any) => {
    if (confirm('Are you sure you want to remove this team member?')) {
        useForm({}).delete(`/projects/team/${member.id}`);
    }
};

const updateMobilizationStatus = (member: any, newStatus: string) => {
    useForm({ status: newStatus }).post(`/projects/team/${member.id}/mobilization`, {
        preserveScroll: true,
    });
};
...
const filteredTeam = computed(() => {
    ...
    return props.team.filter(m => 
        m.employee.first_name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
        m.employee.last_name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
...
const getStatusColor = (status: string) => {
    switch(status) {
        ...
        case 'MCU': return 'bg-purple-100 text-purple-800 border-purple-200';
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
            <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 flex-1">
                    <div>
                        ...
                        <p class="text-sm text-zinc-500 mt-1">Manage manpower allocation and mobilization status for {{ project.name }}</p>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        <div class="flex bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg">
                            <button 
                                ...
                                :class="activeTab === 'list' ? 'bg-white dark:bg-zinc-700 shadow-sm text-zinc-900 dark:text-white' : 'text-zinc-500 hover:text-zinc-700'"
                            >List View</button>
                            <button 
                                @click="activeTab = 'kanban'"
                                class="px-3 py-1.5 text-sm font-medium rounded-md transition-all"
                                :class="activeTab === 'kanban' ? 'bg-white dark:bg-zinc-700 shadow-sm text-zinc-900 dark:text-white' : 'text-zinc-500 hover:text-zinc-700'"
                            >Kanban Board</button>
                        </div>
                                ...
                                <form @submit.prevent="submitBulk" class="space-y-4 mt-2">
                                    <div class="grid gap-2">
                                        ...
                                        <div class="border rounded-md max-h-48 overflow-y-auto p-2 space-y-2">
                                            <div v-for="emp in availableEmployees" :key="emp.id" class="flex items-center gap-2">
                                                <input type="checkbox" :value="emp.id" v-model="bulkForm.employee_ids" class="rounded border-zinc-300" />
                                                <span class="text-sm">{{ emp.first_name }} {{ emp.last_name }} ({{ emp.employee_code }})</span>
                                            </div>
                                            <div v-if="availableEmployees.length === 0" class="text-sm text-zinc-500 text-center py-4">
                                                No available employees found.
                                            </div>
                                        </div>
                                        <span v-if="bulkForm.errors.employee_ids" class="text-xs text-red-500">{{ bulkForm.errors.employee_ids }}</span>
                                    </div>
                                    <div class="grid grid-cols-2 gap-4">
                                        <div class="grid gap-2">
                                            <Label>Role</Label>
                                            <Input v-model="bulkForm.role" placeholder="e.g. Technician" required />
                                        </div>
                                        <div class="grid gap-2">
                                            <Label>Daily Rate</Label>
                                            <Input type="number" v-model="bulkForm.daily_rate_cost" required />
                                        </div>
                                    </div>
                                    <div class="grid gap-2">
                                    ...
                                    <div class="flex justify-end gap-2 pt-2">
                                        <Button type="button" variant="outline" @click="bulkModal = false">Cancel</Button>
                                        <Button type="submit" :disabled="bulkForm.processing">Assign Selected</Button>
                                    </div>
                                </form>
                            </DialogContent>
                        </Dialog>
            ...
            <div v-if="activeTab === 'list'" class="space-y-4">
                <div class="relative">
                    ...
                    <Input v-model="searchQuery" placeholder="Search by name or role..." class="pl-9 max-w-sm" />
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                                ...
                                <tr v-for="member in filteredTeam" :key="member.id" class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                    <td class="px-6 py-4 font-medium text-zinc-900 dark:text-white">
                                        {{ member.employee.first_name }} {{ member.employee.last_name }}
                                        <div class="text-xs text-zinc-500 font-normal">{{ member.employee.employee_code }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border" :class="getStatusColor(member.status || 'RECRUITMENT')">
                                            {{ member.status || 'RECRUITMENT' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        {{ new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(member.daily_rate_cost) }}
                                    </td>
                                    <td class="px-6 py-4 text-right flex justify-end gap-2">
                                        <Button variant="ghost" size="sm" class="h-8 w-8 p-0" @click="openEdit(member)">
                                            <Edit2 class="h-4 w-4" />
                                        </Button>
                                        <Button variant="ghost" size="sm" class="h-8 w-8 p-0 text-red-600 hover:text-red-700 hover:bg-red-50" @click="deleteMember(member)">
            ...
            <!-- Kanban View -->
            <div v-if="activeTab === 'kanban'" class="flex gap-4 overflow-x-auto pb-4 min-h-[600px]">
                <div v-for="stage in mobilizationStages" :key="stage" class="flex-shrink-0 w-80 flex flex-col gap-3">
                    <div class="flex items-center justify-between px-2">
                        <h3 class="font-semibold text-zinc-700 dark:text-zinc-300 text-sm">{{ stage }}</h3>
                        <span class="bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-xs px-2 py-0.5 rounded-full">
                            {{ kanbanData[stage].length }}
                        </span>
                    </div>
                    
                    <div class="bg-zinc-50/50 dark:bg-zinc-900/50 rounded-xl p-2 min-h-[200px] h-full border border-zinc-200 dark:border-zinc-800 flex flex-col gap-2">
                        <div 
                        ...
                        >
                            <div class="flex justify-between items-start mb-1">
                                <span class="font-medium text-sm">{{ member.employee.first_name }} {{ member.employee.last_name }}</span>
                                <Button variant="ghost" size="xs" class="h-6 w-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity" @click="openEdit(member)">
                            ...
                            <!-- Quick Move Actions -->
                            <div class="flex justify-between mt-2 pt-2 border-t border-zinc-100 dark:border-zinc-700">
                                <Button 
                                    v-if="stage !== 'RECRUITMENT'" 
                                    variant="ghost" 
                                    size="xs" 
                                    class="text-[10px] h-6 px-1"
                                    @click="updateMobilizationStatus(member, mobilizationStages[mobilizationStages.indexOf(stage) - 1])"
                                >
                                    ← Prev
                                </Button>
                                <span v-else></span>
                        ...
                        <div v-if="kanbanData[stage].length === 0" class="text-center py-8 text-zinc-400 text-xs italic">
                            No members
                        </div>
                    </div>
                </div>
                ...
                <DialogContent class="sm:max-w-md">
                    ...
                    <form @submit.prevent="submitCreate" class="space-y-4 mt-2">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="grid gap-2">
                                <Label>Role</Label>
                                <Input v-model="form.role" placeholder="e.g. Site Engineer" required />
                            ...
                            <div class="grid gap-2">
                                ...
                                <span v-if="form.errors.daily_rate_cost" class="text-xs text-red-500">{{ form.errors.daily_rate_cost }}</span>
                            </div>
                        </div>
                    ...
                    <form @submit.prevent="submitEdit" class="space-y-4 mt-2">
                        ...
                        <div class="grid gap-2">
                            ...
                            <select 
                                ...
                                class="flex h-10 w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm ring-offset-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:border-zinc-800 dark:bg-zinc-950 dark:ring-offset-zinc-950 dark:placeholder:text-zinc-400 dark:focus-visible:ring-zinc-300"
                            >
                                <option v-for="stage in mobilizationStages" :key="stage" :value="stage">{{ stage }}</option>
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="grid gap-2">
                                <Label>Daily Rate</Label>
                                <Input type="number" v-model="editForm.daily_rate_cost" required />
                            </div>
                        </div>
