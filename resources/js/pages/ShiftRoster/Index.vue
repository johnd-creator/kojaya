...
const monthLabel = computed(() => {
  return new Date(props.year, props.month - 1, 1).toLocaleString("id-ID", {
    month: "long",
    year: "numeric",
  });
});
...
const saveEdit = () => {
  ...
  router.patch(
    `/shift-rosters/${editing.value.id}`,
    {
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-1 flex-col gap-6 p-6 max-w-full mx-auto w-full">
      ...
      >
        <div>
          ...
          <p class="text-zinc-500 mt-1">
            Monthly rotating shift schedule for operational groups A, B, C, and
            D.
          </p>
        </div>
        <div class="flex gap-3 items-center">
          ...
          <span
            class="min-w-[160px] text-center font-semibold text-zinc-900 dark:text-white"
            >{{ monthLabel }}</span
          >
          <Button variant="outline" @click="navigate(1)">
          ...
          >
            <RefreshCw
              class="h-4 w-4 mr-2"
              :class="{ 'animate-spin': generateForm.processing }"
            />
            Generate Roster
          </Button>
        </div>
      ...
      <div class="flex flex-wrap gap-3 text-xs">
        ...
        >
          <span
            ...
            :class="shiftColors[shift.id]?.split(' ')[0]"
          ></span>
        ...
        <div class="flex items-center gap-1.5">
          <span
            class="w-3 h-3 rounded-sm inline-block bg-zinc-200 dark:bg-zinc-700"
          ></span>
      ...
      <!-- Calendar Grid -->
      <div
        class="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm bg-white dark:bg-zinc-900"
      >
        <table class="w-full text-xs min-w-[900px]">
          <thead>
            <tr class="border-b border-zinc-200 dark:border-zinc-800">
              <th
                class="sticky left-0 bg-white dark:bg-zinc-900 px-4 py-3 text-left text-zinc-500 uppercase font-medium w-16 z-10"
              >
                Group
              </th>
              <th
          ...
          <tbody>
            ...
            >
              <td
                class="sticky left-0 bg-white dark:bg-zinc-900 px-4 py-2 z-10"
              >
                <span
                  class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 font-bold text-sm"
                >
                  {{ group }}
                </span>
              </td>
              <td
                ...
                :class="
                  date === today ? 'bg-indigo-50/50 dark:bg-indigo-900/10' : ''
                "
              >
                <button
      ...
      <p class="text-xs text-zinc-400 text-center">
        Click any cell to override the shift assignment for that day.
      </p>
    </div>
    ...
    >
      <div
        class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-xl w-full max-w-sm p-6 space-y-4"
      >
        ...
        <div class="space-y-2">
          <label
            class="text-sm font-medium text-zinc-700 dark:text-zinc-300 flex items-center gap-2"
          >
            <input
              type="checkbox"
              v-model="editing.is_off_day"
              class="rounded"
            />
        ...
        <div class="space-y-2">
          <label class="text-xs font-medium text-zinc-500 block"
