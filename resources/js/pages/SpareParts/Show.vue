...
import { ArrowLeft, Package, AlertTriangle, Home, TrendingUp } from 'lucide-vue-next';
import { ref } from 'vue';
        ...
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-6 max-w-7xl mx-auto w-full">
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <AlertTriangle v-if="sparePart.is_below_min" class="h-4 w-4 text-red-500" />
                        </div>
                    </div>
                    <p class="text-zinc-500 mt-1">
                        Code: <span class="font-mono">{{ sparePart.code }}</span>
                        <span v-if="sparePart.specification" class="ml-3">| {{ sparePart.specification }}</span>
                    </p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <h3 class="font-semibold text-zinc-900 dark:text-white">Stock Overview</h3>
                    </div>
                    <div class="space-y-4">
                        <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-sm font-medium text-zinc-900 dark:text-white">{{ sparePart.min_stock }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-zinc-500">Max Stock</p>
                                <p class="text-sm font-medium text-zinc-900 dark:text-white">{{ sparePart.max_stock }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-zinc-500">Reorder Level</p>
                                <p class="text-sm font-medium text-zinc-900 dark:text-white">{{ sparePart.reorder_level }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-zinc-500">Total Stock</p>
                                <p class="text-sm font-medium text-zinc-900 dark:text-white">{{ sparePart.total_stock }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Part Details -->
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
                    <h3 class="font-semibold text-zinc-900 dark:text-white mb-4">Part Details</h3>
                    <div class="space-y-3">
                        <div>
                        ...
                        <div v-if="sparePart.organization">
                            ...
                            <p class="text-sm font-medium text-zinc-900 dark:text-white">
                                {{ sparePart.organization.name }} ({{ sparePart.organization.code }})
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <h3 class="font-semibold text-zinc-900 dark:text-white">Update Stock</h3>
                    </div>
                    <form @submit.prevent="submitStockUpdate" class="space-y-4">
                        <div class="space-y-2">
                            ...
                            >
                                ...
                                <option v-for="stock in sparePart.stocks" :key="stock.warehouse.id" :value="stock.warehouse.id">
                                    {{ stock.warehouse.name }} ({{ stock.warehouse.code }})
                                </option>
                            </select>
                            <p v-if="stockForm.errors.warehouse_id" class="text-sm text-red-500">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <Input
                                    ...
                                    required
                                />
                                <p v-if="stockForm.errors.quantity" class="text-sm text-red-500">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="font-semibold text-zinc-900 dark:text-white">Warehouse Stock</h3>
                </div>
                <div v-if="sparePart.stocks.length === 0" class="text-center py-8 text-zinc-500">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-zinc-200 dark:border-zinc-800">
                                <th class="py-3 px-4 font-medium text-sm text-zinc-500">Warehouse</th>
                                <th class="py-3 px-4 font-medium text-sm text-zinc-500">Available</th>
                                <th class="py-3 px-4 font-medium text-sm text-zinc-500">Reserved</th>
                                <th class="py-3 px-4 font-medium text-sm text-zinc-500">Total</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                            <tr v-for="stock in sparePart.stocks" :key="stock.id">
                                <td class="py-3 px-4">
                                    ...
                                    <span class="text-xs text-zinc-500 ml-2">({{ stock.warehouse.code }})</span>
                                </td>
                                <td class="py-3 px-4">
