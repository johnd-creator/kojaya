...
import { ArrowLeft, CheckCircle2, Circle, ClipboardList, User, Wrench } from 'lucide-vue-next';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: props.workOrder.id.slice(0, 8), href: `/work-orders/${props.workOrder.id}` },
];

const statusColor = (status: WorkOrder['status']) => {
    const colors: Record<WorkOrder['status'], string> = {
        OPEN: 'bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-900/30 dark:text-sky-400',
...
const checklistDone = computed(() => checklists.value.filter(i => i.is_checked).length);
        ...
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-6 max-w-7xl mx-auto w-full">
            <div class="flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2">
                    <div class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium border"
                    ...
                    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ workOrder.type }}</div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ formatDateTime(workOrder.created_at) }}</div>
                            </div>
                            <div>
                                <div class="text-zinc-500">Completed</div>
                                <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ formatDateTime(workOrder.completed_at) }}</div>
                            </div>
                        </div>

                        <div class="mt-5">
                            <div class="text-sm text-zinc-500 mb-2">Description</div>
                            <div v-if="workOrder.description" class="rounded-lg border border-zinc-200 dark:border-zinc-800 p-4 text-sm text-zinc-700 dark:text-zinc-200">
                                {{ workOrder.description }}
                            </div>
                            <div v-else class="rounded-lg border border-dashed border-zinc-200 dark:border-zinc-800 p-4 text-sm text-zinc-500">
                                No description
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
                        <div class="flex items-center justify-between gap-4 mb-4">
                            <div class="flex items-center gap-2">
                                ...
                                <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Checklist</h2>
                            </div>
                        ...
                        <div class="h-2 rounded-full bg-zinc-100 dark:bg-zinc-800 overflow-hidden">
                            <div class="h-full bg-emerald-500" :style="{ width: `${checklistPercent}%` }"></div>
                        </div>

                        <div v-if="checklistTotal === 0" class="py-10 text-center text-zinc-500">
                            No checklist assigned to this work order.
                        </div>
                        <div v-else class="mt-4 divide-y divide-zinc-100 dark:divide-zinc-800 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                            <div v-for="item in checklists" :key="item.id" class="p-4 flex items-start gap-3">
                                <div class="mt-0.5">
                                    <CheckCircle2 v-if="item.is_checked" class="h-5 w-5 text-emerald-600" />
                                    <Circle v-else class="h-5 w-5 text-zinc-300 dark:text-zinc-600" />
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-4">
                                        <div>
                                            ...
                                            <div v-if="item.item_description" class="text-sm text-zinc-500 mt-0.5">
                                                {{ item.item_description }}
                                            </div>
                                        </div>
                                        <div v-if="item.is_checked" class="text-xs text-zinc-500 text-right shrink-0">
                                            <div>{{ item.checkedBy?.name || 'Checked' }}</div>
                                            <div>{{ formatDateTime(item.checked_at) }}</div>
                                        </div>
                                    </div>
                                    <div v-if="item.notes" class="mt-2 text-sm text-zinc-700 dark:text-zinc-200 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">
                                        {{ item.notes }}
                                    </div>
                                </div>
                            </div>
                ...
                <div class="space-y-6">
                    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Assignment</h2>
                        </div>
                        <div class="text-sm">
                            ...
                            <div v-if="workOrder.assignedTo?.email" class="text-zinc-500 mt-1">
